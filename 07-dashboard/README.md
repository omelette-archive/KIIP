...
[`web/`](web/)은 요약/지역별 출원율/지자체별 조회/품목별 조회/지역 출원 미확인/
특화작목 비교/데이터 개요의 7개 탭을 제공한다. 판정 기준은 데이터 개요에서 확인할 수 있고,
지역·품목 상세, 출원 상표 사례와 출처 표시가 동작한다. 운영 자동화는 #70에서 관리한다.
...
- **"특화작목(농촌진흥청 지정)"** — 2025년 공식 보도자료 붙임3에서 9개 도·69개
  작목(대표 9·집중육성 18·자체육성 42)을 구조화했다. 2026-08-26 #117 후속으로
  모든 작목을 도 단위 공식 특산품 수집원으로 승격하고 KIPRIS·출원인 주소 대조와
  `특화작목 비교` 탭의 도별 출원·미출원·집계대기 현황에 반영한다.
...
- [x] 샘플 스냅샷을 읽는 웹 UI, 전국 지도, 지역·품목 조회, 특화작목 비교 준비 화면,
      근거·부분수집·차단 상태 표시
- [x] 판정 기준·매칭 방법을 데이터 개요 탭의 "판정 기준과 매칭 방법" 섹션에 통합 —
      품목·지역 매칭/상표 검색/지역 주소 일치 출원/출원인 지역 매칭 기준을 근거와 함께 표시.
...
- [x] `데이터 개요` 탭에서 수집 입력 → 표준화 → 고유 검색어 → 상표 매칭 → 지역별 집계 흐름과
      현재 스냅샷의 주소 확보율을 동적으로 표시. 요약 화면의 모호한 상표 사례 카드는 제거함
- [ ] 실키 수집 스케줄과 사이트 무인 재배포 연결(#70)
...
현재 수치는 각 특산품에 매핑된 고시상품 NICE류 중심의 제한적 집계다. 예를 들어 굴비가
29류로 매핑되어 있으면 29류 결과를 집계하며, 음식점업 43류·도소매업 35류 등 서비스류는
포함하지 않는다. 서비스류 분석은 현행 원물 중심 지표와 섞지 않고 후속 확장 범위로 관리한다.
...
않는다. 검토 실행은 `--stage alpha`와 상한 실험 메타데이터를 함께 남겨 공식 운영 결과와
구분한다. 예:

```powershell
node 07-dashboard/buildDashboardSnapshot.js `
  --analysis 04-analyze-brand/output/<run-id>-analysis.json `
  --gap 05-detect-brand-gap/output/<run-id>-gap.json `
  --strategy 06-generate-business-strategy/output/<run-id>-strategy.json `
...
현재 스냅샷은 수집 완료 여부를 지역 지표 공개 기준으로 사용한다. 출원인 주소
