# 지역 특산품-상표 분석·정책지원 플랫폼 웹

`dashboard-snapshot-v1`을 직접 읽는 ⑦단계 웹 대시보드다. 현재 입력은 전체 범위에서 실행한
스냅샷이며 화면 상단에는 `전체 데이터`와 생성일을 표시한다. 부분 수집·지역 집계 대기 상태는
해당 품목과 지표에서 구분한다. 전국 검색 후보와 지역 귀속 완료 수치를 섞지 않고 입력행·고유
검색 조합·주소 검증·지역 지표 게이트를 별도 단위로 보여준다.

현재 화면은 `요약 / 지역별 출원율 / 지자체별 조회 / 품목별 조회 / 지역 출원 미확인 /
특화작목 비교 / 데이터 개요`의 7개 탭으로 구성된다. 요약 지도는
...
`pipelineStatus.stage=alpha`는 공식 운영 통계가 아닌 검토 실행임을 뜻한다. 스냅샷 파일에는 출처·계약 버전·생성
...
현재 실키 자동 수집 주기와 승인 게시 연결은 #70의 잔여 범위다.
