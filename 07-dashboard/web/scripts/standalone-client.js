...
function dashboardClient(snapshot, geometry) {
  ...
  function summaryScreen() {
    ...
    <section class="map-workspace"><div class="map-card"><div class="map-heading"><div><p class="eyebrow">BRAND GAP MAP</p><h2>${state.province ? `${esc(state.province)} 시군구` : "전국 지역 브랜드 지도"}</h2></div><span class="reference-chip">참고 경계 · 2013 KOSTAT</span></div><div class="map-toolbar"><div class="map-metrics">${Object.entries(mapLabels).map(([key, label]) => `<button type="button" data-map-metric="${key}" class="${state.mapMetric === key ? "active" : ""}" ${key === "gap" && !hasGap ? 'disabled title="현재 공백 점수는 확인할 수 없습니다."' : ""}>${label}</button>`).join("")}</div>${state.province ? '<button class="map-back" id="map-back" type="button">← 전국</button>' : ""}</div><div class="map-stage"><svg class="korea-map" viewBox="${municipal?.viewBox || geometry.viewBox}" role="img" aria-label="${state.province ? `${esc(state.province)} 시군구 지도` : "대한민국 시도 지도"}">${shapes}</svg></div><div class="map-legend"><span><i class="legend-swatch no-data"></i>데이터 없음</span><span><i class="legend-swatch low"></i>낮음</span><span><i class="legend-swatch high"></i>높음</span><strong>${mapLabels[state.mapMetric]} 기준</strong></div><p class="map-warning">${esc(geometry.boundaryReference.warning)} 공백 점수는 #29 기준과 값이 준비될 때만 활성화됩니다.</p></div>
    <aside class="map-insight"><p class="eyebrow">SELECTED AREA</p><h2>${esc(state.municipality || state.province || "전국")}</h2><p class="insight-summary">${state.province ? `${visibleRegions.length}개 수집 지역, ${visibleItems.length}개 특산품` : `${snapshot.coverage.observedRegionCount}개 관측 지역 · 검증 완료 항목만 수치 반영`}</p><div class="mini-list">${visibleItems.slice(0, 5).map(({ region, item }) => `<button type="button" data-open-region="${esc(regionKey(region))}" data-open-item="${esc(item.specialtyId || "")}"><span><strong>${esc(region.sigungu || region.region)} / ${esc(officialItemLabel(item))}</strong><small>NICE ${esc(item.niceClass)}류</small></span><b>${item.metrics.uniqueTrademarkCount.availability === "available" ? `출원 ${number(item.metrics.uniqueTrademarkCount.value)}건` : "데이터 검토 중"}</b></button>`).join("") || '<p class="empty">이 지역은 아직 고시명칭이 확정된 품목이 없습니다.</p>'}</div><div class="insight-note"><strong>집계 원칙</strong><p>고시명칭이 확정된 특산물만 표시합니다. 등록 브랜드·상표명은 지역 상세와 "수집된 상표 예시"에서 개별 사례로 확인할 수 있습니다.</p></div></aside></section>
    <section class="ranking" aria-label="등록상표 랭킹"><div class="section-heading"><div><p class="eyebrow">TRADEMARK RANKING</p><h2>지역×대표 특산품 등록상표 랭킹</h2></div><div class="ranking-toggle">${[10, 50].map((limit) => `<button type="button" data-ranking-limit="${limit}" class="${state.rankingLimit === limit ? "active" : ""}">TOP ${limit}</button>`).join("")}</div></div><p class="ranking-note">대표 특산품명으로 표시하고 고시명칭·NICE류를 집계 근거로 병기합니다. 수집 및 지역 귀속 검증 기준을 통과한 항목만 순위에 포함합니다.</p><div class="ranking-table-wrap"><table class="ranking-table"><thead><tr><th>순위</th><th>지역</th><th>대표 특산품</th><th>고시명칭·NICE</th><th>등록상표</th></tr></thead><tbody>${rankingRows.slice(0, state.rankingLimit).map(({ region, item }, index) => `<tr><td>${index + 1}</td><td>${esc(region.region)}</td><td>${esc(itemName(item))}</td><td>${esc(officialNoticeName(item) || "미확정")} · ${esc(item.niceClass || "—")}류</td><td>${number(item.metrics.registeredTrademarkCount.value)}건</td></tr>`).join("")}</tbody></table></div></section>
  ...
  function itemsScreen() {
    const rows = itemRows(); return `<section class="screen-section"><div class="screen-heading"><div><p class="eyebrow">ITEM EXPLORER</p><h1>품목별 조회</h1></div><p>품목을 기준으로 지역과 상표 활동을 다시 묶었습니다.</p></div><div class="item-screen"><div class="item-screen-toolbar"><label><span class="sr-only">품목 검색</span><input id="item-search" value="${esc(state.itemQuery)}" placeholder="품목 또는 지역 검색"></label><span>${rows.length}개 품목</span></div><div class="item-table"><div class="item-table-head"><span>품목</span><span>수집 지역</span><span>지역 고유 상표</span><span>지역 등록 상표</span><span>등록률</span></div>${rows.map((row, index) => `<div class="item-table-row"><span><b>${String(index + 1).padStart(2, "0")}</b><strong>${esc(row.name)}</strong></span><span>${esc(row.regions.join(", "))}</span><span>${row.verified ? `${number(row.trademarks)}건` : "검증 중"}</span><span>${row.verified ? `${number(row.registered)}건` : "검증 중"}</span><span>${row.verified ? percent(row.trademarks ? row.registered / row.trademarks : null) : "—"}</span></div>`).join("")}</div><div class="method-note"><strong>집계 기준</strong><p>고시명칭·NICE류가 확정된 품목만 공식 명칭 기준으로 재그룹합니다("풋고추"·"파프리카"는 같은 "고추"로 합쳐짐). 아직 고시명칭이 확정되지 않은 검토대기 원물명은 지역별 상세 화면에서 원문 그대로 확인할 수 있습니다.</p></div></div></section>`;
  }
  function dataScreen() {
  ...
  function bind() {
    ...
    bindSearchInput(document.querySelector("#region-search"), (value) => { state.query = value; });
    bindSearchInput(document.querySelector("#item-search"), (value) => { state.itemQuery = value; });
  }
  // render()가 #app 전체를 innerHTML로 다시 그리면서 input DOM 노드 자체를 파괴·재생성한다.
  // 한글은 자음+모음을 조합해 한 음절을 완성하는 IME 조합 입력이라, 조합 중(예: "ㅎ"→"하"→"한")에
  // 이 노드가 교체되면 브라우저의 조합 상태가 끊겨 타이핑이 씹히거나 마지막 자모만 남는다.
  // compositionstart~compositionend 사이에는 절대 다시 그리지 않고, 조합이 끝난 뒤에만
  // 반영한다(event.isComposing으로도 동일하게 판단하되 구형 브라우저 대비 플래그를 별도로 둔다).
  function bindSearchInput(input, apply) {
    ...
    input.addEventListener("input", (event) => { if (event.isComposing || composing) return; commit(); });
  }
  function render() {
    nav();
