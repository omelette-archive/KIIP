...
function dashboardClient(snapshot, geometry, registrationExamples) {
  ...
  const regionTrendHtml = (region, heading = "연도별 출원·등록 추이", subtitle, options = {}) => {
    ...
    if (!allYears.length) return `<section class="${wrapClass}"><div class="section-heading"><div><h2>${esc(heading)}</h2></div><span>${esc(region.region)}</span></div><p class="empty">${esc(emptyLabel)}</p></section>`;
  ...
  function summaryScreen() {
    ...
    currentCsvExporters.summaryApplicationRanking = () => downloadCsv(`지역대표특산품출원랭킹_${csvDateStamp(dashboardUpdatedAt)}`, ["순위", "지역", "대표 특산품", "출원 확인"], applicationRankingRows.slice(0, RANKING_LIMIT).map(({ region, item, label }, index) => [index + 1, region.region, label, item.metrics.uniqueTrademarkCount.value ?? 0]));
    currentCsvExporters.summaryRegistrationRanking = () => downloadCsv(`지역대표특산품등록랭킹_${csvDateStamp(dashboardUpdatedAt)}`, ["순위", "지역", "대표 특산품", "등록 완료"], registrationRankingRows.slice(0, RANKING_LIMIT).map(({ region, item, label }, index) => [index + 1, region.region, label, item.metrics.registeredTrademarkCount.value ?? 0]));
  ...
  function regionDetail(region, item) {
    ...
    return `<div class="detail-panel">
      ${heading}
      ${regionTrendHtml(region, "지역 연도별 출원·등록 추이", `${region.region} 전체 특산품 · 연도별`, { prominent: true })}
  ...
  function strategyScreen() {
    ...
    if (selected) {
      ...
      bodyHtml = `${samples.length === 0 ? '<p class="empty">아직 표시할 샘플이 없습니다.</p>' : ""}<div class="strategy-sample-list">${samples.map(({ region, item }) => businessStrategyCardHtml(item.briefing, `${region.region} · ${itemName(item)}`, ` <button type="button" class="strategy-jump-link" data-open-region="${esc(regionKey(region))}" data-open-item="${esc(item.specialtyId || "")}">지자체별 조회에서 자세히 보기 →</button>`)).join("")}</div>`;
    }
  ...
  function compareScreen() {
    ...
    const regionSectionHtml = `<section class="compare-region-section"><div class="compare-section-head"><div><span>69개 전체 상세</span><h2>등급별 특화작목 출원 현황</h2></div><p>상표 출원건수는 각 특화작목의 원물명 검색 기준 지역 주소 일치 출원 합계, 출원 비율은 출원이 1건 이상 확인된 작목 비율입니다.</p>${csvDownloadButtonHtml("compareRegion")}</div><div class="compare-region-table"><div class="compare-region-head"><span>지역</span><span>대표작목</span><span>자체육성작목</span><span>집중육성작목</span><span>상표 출원건수<small>원물 기준</small></span><span>출원 비율</span><span title="판정이 끝난 작목 수 / 전체 지정 작목 수">수집 진행</span></div>${rowsHtml}</div></section>`;
