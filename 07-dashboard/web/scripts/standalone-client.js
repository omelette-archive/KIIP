...
function dashboardClient(snapshot, geometry) {
  ...
  function provinceValue(name, metric = state.mapMetric) {
    ...
    return row.officialItems ? row.appliedItems / row.officialItems : null;
  }
  ...
  function summaryScreen() {
    ...
    <aside class="map-insight"><h2>${esc(displayRegionName(state.municipality || state.province || "전국"))}</h2><p class="insight-summary">확인 특산품 ${number(visibleSpecialtyCoverage.total)}개 중 출원 확인 ${number(visibleSpecialtyCoverage.applied)}개 · 출원율 ${percent(visibleSpecialtyCoverage.rate)} · 판정 완료 ${number(visibleSpecialtyCoverage.decided)}개${visibleSpecialtyCoverage.pending ? ` · 집계 대기 ${number(visibleSpecialtyCoverage.pending)}개` : ""}</p><div class="mini-list">${visibleItems.slice(0, 5).map(({ region, item, label }) => `<button type="button" data-open-region="${esc(regionKey(region))}" data-open-item="${esc(item.specialtyId || "")}"><span><strong>${esc(region.sigungu || region.region)} / ${esc(label)}</strong><small>${esc(noticeBasis(item))} · NICE ${esc(item.niceClass)}류</small></span><b>${item.metrics.uniqueTrademarkCount.availability === "available" ? (item.metrics.uniqueTrademarkCount.value || 0) > 0 ? `출원 확인 · ${number(item.metrics.uniqueTrademarkCount.value)}건` : "출원 없음 · 판정 완료" : "지역별 집계 대기"}</b></button>`).join("") || '<p class="empty">이 지역에는 고시명칭이 확인된 특산품이 없습니다.</p>'}</div><div class="insight-note"><strong>출원율 계산</strong><p>출원 확인 특산품 수 ÷ 이 지역의 확인 특산품 전체 수입니다(2026-08-20 계산 방식 변경). 판정 완료·집계 대기 수를 함께 표시하며, 아직 집계가 안 끝난 대기 품목은 출원 미확인으로 계산합니다 — 데이터가 쌓일수록 값이 올라갈 수 있습니다.</p></div></aside></section>
  ...
  // 2026-08-21: 법정동코드가 미해결(region.regionCode 없음)인 지역은 제외한다 — 지역
  // 코드가 없으면 ③단계 출원인 주소-지역 일치 판정(inside/outside)이 그 지역에 대해
  // 구조적으로 성립하지 않아, "0건"이 실제 미출원이 아니라 판정 불가일 뿐이다(대전·
  // 대구·부산·울산·인천광역시, 전남광주통합특별시, 남양주시 등 7개 지역 확인, 사용자 결정).
  function gapRows() {
  ...
  function compareScreen() {
    ...
    return `<section class="screen-section"><div class="screen-heading"><div><h1>특화작목 비교</h1></div><p>지금 비교할 수 있는 데이터와 아직 필요한 데이터를 먼저 구분했습니다.</p></div><div class="compare-banner"><span>현재 단계</span><strong>비교 기준 원본 확보 전 · 준비 현황만 확인 가능</strong><p>정책 지정 특화작목 목록이 아직 없어 일치율은 계산하지 않습니다. 현재 지역 특산품과 출원 현황은 아래에서 먼저 확인할 수 있습니다.</p></div><div class="compare-readiness"><article class="ready"><span>01 · 현재 보유</span><strong>지역 특산품·상표 현황</strong><p>지역별 확인 특산품 수, 출원 판정 수와 출원율</p></article><i>→</i><article class="waiting"><span>02 · 추가 필요</span><strong>정책 지정 특화작목 원본</strong><p>지정 지역·작목·기간·근거 문서</p></article><i>→</i><article><span>03 · 원본 확보 후</span><strong>일치·누락 비교</strong><p>정책 작목 대비 상표 활동과 미출원 품목</p></article></div><section class="compare-region-section"><div class="compare-section-head"><div><span>현재 확인 가능</span><h2>지역별 특산품 출원 현황</h2></div><p>정책 비교 결과가 아니라, 비교에 투입될 현재 데이터입니다.</p></div><div class="compare-region-table"><div class="compare-region-head"><span>지역</span><span>전체 확인 특산품</span><span>전체 특산품 출원율</span><span>정책 비교</span></div>${comparisonRows.map(({ province, coverage, names }) => `<div class="compare-region-row"><strong>${esc(province)}</strong><div><b>${number(coverage.total)}개</b></div><div><b>${percent(coverage.rate)}</b><small>전체 ${coverage.total}개 중 출원 확인 ${coverage.applied}개 · 판정 완료 ${coverage.decided}개${coverage.pending ? ` · ${coverage.pending}개 대기` : ""}</small></div><span class="compare-waiting">원본 대기</span><details class="compare-items-detail"><summary>확인된 특산품 ${number(names.length)}개 보기</summary><div class="compare-item-chips">${names.map((name) => `<span>${esc(name)}</span>`).join("")}</div></details></div>`).join("")}</div></section><div class="compare-sources"><article><span>필수 입력</span><strong>농촌진흥청 지역특화작목 지정 목록</strong><p>지역·작목·계획 기간·근거 버전을 구조화해야 합니다.</p></article><article><span>처리 원칙</span><strong>원본 확보 후 자동 비교</strong><p>명칭 정규화 후보만 개별 검토하고 집계·일치 판정은 자동화합니다.</p></article></div></section>`;
  }
