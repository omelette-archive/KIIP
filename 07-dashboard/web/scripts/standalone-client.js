...
function dashboardClient(snapshot, geometry) {
  ...
  function itemsScreen() {
    const rows = itemRows(); return `<section class="screen-section"><div class="screen-heading"><div><p class="eyebrow">ITEM EXPLORER</p><h1>품목별 조회</h1></div><p>품목을 기준으로 지역과 상표 활동을 다시 묶었습니다.</p></div><div class="item-screen"><div class="item-screen-toolbar"><label><span class="sr-only">품목 검색</span><input id="item-search" value="${esc(state.itemQuery)}" placeholder="품목 또는 지역 검색"></label><span>${rows.length}개 품목</span></div><div class="item-table-explainer"><strong>표의 수치 읽는 법</strong><p><b>지역 주소 일치 출원</b>은 같은 품목의 각 확인 지역에서 출원인 주소가 그 지역으로 판정된 고유 출원을 합한 값입니다. <b>그중 등록</b>은 해당 출원 중 현재 등록 상태인 건수이며, <b>등록 비율 = 그중 등록 ÷ 지역 주소 일치 출원</b>입니다. 전국 검색 결과 전체가 아닙니다.</p><p><b>지역별 집계 대기</b>는 특산품명이 미확정이거나 상표가 0건이라는 뜻이 아닙니다. 이 표의 품목명은 이미 고시명칭·NICE류 확인을 마쳤고, 상표 검색이 일부만 수집된 경우 지역별 수치를 확정하지 않고 대기 상태로 표시합니다.</p></div><div class="item-table"><div class="item-table-head"><span>품목</span><span>특산품 확인 지역</span><span>지역 주소 일치 출원</span><span>그중 등록</span><span>등록 비율</span></div>${rows.map((row, index) => { const regionPreview = esc(row.regions.slice(0, 3).join(", ")); const regionMore = row.regions.length > 3 ? ` 외 ${row.regions.length - 3}곳` : ""; return `<div class="item-table-row"><span><b>${String(index + 1).padStart(2, "0")}</b><strong>${esc(row.name)}</strong></span><span class="metric-cell" title="${esc(row.regions.join(", "))}"><strong>${row.regions.length}개 지역</strong><small>${regionPreview}${regionMore}</small></span><span class="metric-cell ${row.trademarksDisplay ? "" : "pending-value"}"><strong>${row.trademarksDisplay ? `${number(row.trademarksDisplay)}건${row.hasProvisional ? " (잠정 포함)" : ""}` : "집계 대기"}</strong></span><span class="metric-cell ${row.available ? "" : "pending-value"}"><strong>${row.available ? `${number(row.registered)}건` : "집계 대기"}</strong></span><span class="metric-cell ${row.available ? "" : "pending-value"}"><strong>${row.available ? row.trademarks ? percent(row.registered / row.trademarks) : "계산 불가" : "집계 대기"}</strong></span></div>`; }).join("")}</div><div class="method-note"><strong>품목명 집계 기준</strong><p>고시명칭·NICE류가 확정된 품목만 공식 명칭 기준으로 재그룹합니다("풋고추"·"파프리카"는 같은 "고추"로 합쳐짐). 아직 고시명칭이 확정되지 않은 원물명은 이 표에서 제외하고 지역별 상세 화면에 원문 그대로 보존합니다.</p></div></div></section>`;
  }
  function dataScreen() {
