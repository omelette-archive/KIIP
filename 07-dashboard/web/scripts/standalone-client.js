...
function dashboardClient(snapshot, geometry) {
  ...
  // 전국 검색 후보 건수를 잠정치로 보여준다("(잠정)" 표시). 등록 여부·등록률·출원율
  // (specialtyCoverage)은 지역 확인이 끝난 건만 계산할 수 있는 근거라 잠정치가 없고
  // 그대로 "집계 대기"로 둔다 — 여기서 바꾸는 것은 상표 "건수" 표시뿐이다.
  ...
  function summaryScreen() {
    ...
    return `${criteriaHtml()}<section class="hero"><div><p class="eyebrow">LOCAL BRAND OBSERVATORY</p><h1>지역 특산품 상표 분석</h1><p class="hero-copy">지역별 특산품과 관련 상표 현황을 한눈에 확인합니다.</p></div><div class="hero-note"><span>DATA COVERAGE</span><strong>${snapshot.coverage.observedRegionCount}개 지역 · ${snapshot.coverage.regionItemCount}개 지역×품목</strong><p>현재 확인 가능한 데이터 범위입니다.</p></div></section>
  ...
  function itemsScreen() {
    const rows = itemRows(); return `<section class="screen-section"><div class="screen-heading"><div><p class="eyebrow">ITEM EXPLORER</p><h1>품목별 조회</h1></div><p>품목을 기준으로 지역과 상표 활동을 다시 묶었습니다.</p></div><div class="item-screen"><div class="item-screen-toolbar"><label><span class="sr-only">품목 검색</span><input id="item-search" value="${esc(state.itemQuery)}" placeholder="품목 또는 지역 검색"></label><span>${rows.length}개 품목</span></div><div class="item-table-explainer"><strong>표의 수치 읽는 법</strong><p><b>지역 주소 일치 출원</b>은 같은 품목의 각 확인 지역에서 출원인 주소가 그 지역으로 판정된 고유 출원을 합한 값입니다. <b>그중 등록</b>은 해당 출원 중 현재 등록 상태인 건수이며, <b>등록 비율 = 그중 등록 ÷ 지역 주소 일치 출원</b>입니다. 전국 검색 결과 전체가 아닙니다.</p><p><b>지역별 집계 대기</b>는 특산품명이 미확정이거나 상표가 0건이라는 뜻이 아닙니다. 이 표의 품목명은 이미 고시명칭·NICE류 확인을 마쳤고, 상표 검색이 일부만 수집된 경우 지역별 수치를 확정하지 않고 대기 상태로 표시합니다.</p></div><div class="item-table"><div class="item-table-head"><span>품목</span><span>특산품 확인 지역</span><span>지역 주소 일치 출원</span><span>그중 등록</span><span>등록 비율</span></div>${rows.map((row, index) => { const pending = row.regions.length - row.available; const basis = pending > 0 ? `확정 ${row.available}/${row.regions.length}개 지역 · ${pending}개 대기` : `${row.regions.length}개 지역 합계`; return `<div class="item-table-row"><span><b>${String(index + 1).padStart(2, "0")}</b><strong>${esc(row.name)}</strong></span><span>${esc(row.regions.join(", "))}</span><span class="metric-cell ${row.trademarksDisplay ? "" : "pending-value"}"><strong>${row.trademarksDisplay ? `${number(row.trademarksDisplay)}건${row.hasProvisional ? " (잠정 포함)" : ""}` : "집계 대기"}</strong><small>${row.available ? esc(basis) : row.hasProvisional ? `${row.regions.length}개 지역 전국 검색 잠정치 · 지역 확인 진행 중` : `${row.regions.length}개 지역 검색 부분 수집`}</small></span><span class="metric-cell ${row.available ? "" : "pending-value"}"><strong>${row.available ? `${number(row.registered)}건` : "집계 대기"}</strong><small>${row.available ? "위 출원 중 등록 상태" : "지역 출원 수 확정 후 계산"}</small></span><span class="metric-cell ${row.available ? "" : "pending-value"}"><strong>${row.available ? row.trademarks ? percent(row.registered / row.trademarks) : "계산 불가" : "집계 대기"}</strong><small>${row.available ? row.trademarks ? `${number(row.registered)} ÷ ${number(row.trademarks)}` : "출원 0건으로 분모 없음" : "지역 출원 수 확정 후 계산"}</small></span></div>`; }).join("")}</div><div class="method-note"><strong>품목명 집계 기준</strong><p>고시명칭·NICE류가 확정된 품목만 공식 명칭 기준으로 재그룹합니다("풋고추"·"파프리카"는 같은 "고추"로 합쳐짐). 아직 고시명칭이 확정되지 않은 원물명은 이 표에서 제외하고 지역별 상세 화면에 원문 그대로 보존합니다.</p></div></div></section>`;
  }
  function dataScreen() {
