...
function dashboardClient(snapshot, geometry, registrationExamples) {
  ...
  function leaderboardHtml() {
    ...
    const surgeCard = `<article class="leader-card"><div class="leader-card-head"><h4>출원 급증 품목</h4><span class="leader-card-note">직전 기간 대비</span></div>${lb.surging.length === 0 ? `<p class="empty">뚜렷한 급증 품목이 없습니다(최소 출원 ${lb.minBase}건).</p>` : `<ol class="leader-list">${lb.surging.map((row, index) => `<li><button type="button" title="직전 ${number(row.priorApp)}건 → 최근 ${number(row.app)}건" data-goto-item="${esc(row.name)}"><span class="leader-rank">${index + 1}</span><span class="leader-name">${esc(row.name)}${tag(row.category)}</span><b class="leader-val leader-val-wide">${row.fresh ? '<em class="leader-fresh">신규</em>' : `<em class="leader-growth">×${row.growth >= 10 ? Math.round(row.growth) : row.growth.toFixed(1)}</em>`}</b></button></li>`).join("")}</ol>`}</article>`;
