...
function dashboardClient(snapshot, geometry) {
  ...
  const state = { tab: "summary", query: "", itemQuery: "", regionKey: snapshot.regions[0]?.regionCode || snapshot.regions[0]?.region, itemId: "", mapMetric: "trademarks", province: null, municipality: null, rankingLimit: 10, expandedItemRegions: new Set() };
  ...
  // 2026-08-19 결정: 지역 확인이 아직 끝나지 않은 항목은 "집계 대기"로 가리지 않고,
  // 전국 검색 건수를 대신 보여준다("(전국 검색)" 표시). "잠정치"라는 말은 마치
  // 지역 수치의 근사값처럼 들리지만 실제로는 전혀 다른 모집단(전국)의 값이라
  // 출처를 그대로 이름 붙인다. 등록 여부·등록률·출원율(specialtyCoverage)은
  // 지역 확인이 끝난 건만 계산할 수 있는 근거라 대체값이 없고 그대로 "집계
  // 대기"로 둔다 — 여기서 바꾸는 것은 상표 "건수" 표시뿐이다.
  ...
  const regionTradeSummary = (region) => {
    ...
    region.items.forEach((item) => {
      ...
      hasValue = true;
      total += trade.value;
      if (trade.provisional) hasProvisional = true;
    });
    return hasValue ? { total, hasProvisional } : null;
  };
  ...
  snapshot.regions.forEach((region) => {
    ...
    const row = provinceStats.get(name) || { trademarks: 0, trademarksDisplay: 0, hasTrademarkData: false, registered: 0, verified: 0, officialItems: 0, decidedItems: 0, appliedItems: 0 };
    region.items.forEach((item) => {
      const official = Boolean(officialItemLabel(item));
      if (official) row.officialItems += 1;
      const trade = tradeDisplay(item);
      if (trade.value !== null) { row.trademarksDisplay += trade.value; row.hasTrademarkData = true; }
  ...
  function provinceValue(name) {
    ...
    if (state.mapMetric === "trademarks") return row.hasTrademarkData ? row.trademarksDisplay : null;
    if (state.mapMetric === "registration") return row.verified && row.trademarks ? row.registered / row.trademarks : null;
    if (state.mapMetric === "coverage") return row.officialItems;
    return row.decidedItems ? row.appliedItems / row.decidedItems : null;
  }
  function regionValue(region) {
    if (!region) return null;
    if (state.mapMetric === "trademarks") return regionTradeSummary(region)?.total ?? null;
    ...
    if (state.mapMetric === "registration" && verified.length === 0) return null;
    if (state.mapMetric === "registration") return trademarks ? registered / trademarks : 0;
    const coverage = specialtyCoverage([region]);
    if (state.mapMetric === "coverage") return coverage.total;
    return coverage.rate;
  }
  function mapValueLabel(value) { if (value === null) return "데이터 없음"; if (["registration", "applicationCoverage"].includes(state.mapMetric)) return percent(value); return `${number(value)}${state.mapMetric === "trademarks" ? "건" : "개 품목"}`; }
  ...
  function itemRows() {
    ...
    snapshot.regions.forEach((region) => region.items.forEach((item) => {
      ...
      const row = rows.get(name) || { name, trademarks: 0, trademarksDisplay: 0, hasProvisional: false, registered: 0, available: 0, regions: [] };
      const trade = tradeDisplay(item);
    ...
    return [...rows.values()].filter((row) => !keyword || row.name.toLocaleLowerCase("ko-KR").includes(keyword) || row.regions.some((region) => region.toLocaleLowerCase("ko-KR").includes(keyword))).sort((a, b) => b.trademarksDisplay - a.trademarksDisplay);
  }
  function itemsScreen() {
    const rows = itemRows(); const ITEM_ROW_LIMIT = 100; const visibleRows = rows.slice(0, ITEM_ROW_LIMIT); return `<section class="screen-section"><div class="screen-heading"><div><p class="eyebrow">ITEM EXPLORER</p><h1>품목별 조회</h1></div><p>품목을 기준으로 지역과 상표 활동을 다시 묶었습니다.</p></div><div class="item-screen"><div class="item-screen-toolbar"><label><span class="sr-only">품목 검색</span><input id="item-search" value="${esc(state.itemQuery)}" placeholder="품목 또는 지역 검색"></label><span>${rows.length > ITEM_ROW_LIMIT ? `상표 출원 건수 상위 ${ITEM_ROW_LIMIT}개 표시 (전체 ${rows.length}개 품목)` : `${rows.length}개 품목`}</span></div><div class="item-table-explainer"><strong>표의 수치 읽는 법</strong><p><b>지역 주소 일치 출원</b>은 같은 품목의 각 확인 지역에서 출원인 주소가 그 지역으로 판정된 고유 출원을 합한 값입니다. <b>그중 등록</b>은 해당 출원 중 현재 등록 상태인 건수이며, <b>등록 비율 = 그중 등록 ÷ 지역 주소 일치 출원</b>입니다. 전국 검색 결과 전체가 아닙니다.</p><p><b>지역별 집계 대기</b>는 특산품명이 미확정이거나 상표가 0건이라는 뜻이 아닙니다. 이 표의 품목명은 이미 고시명칭·NICE류 확인을 마쳤고, 상표 검색이 일부만 수집된 경우 지역별 수치를 확정하지 않고 대기 상태로 표시합니다.</p></div><div class="item-table"><div class="item-table-head"><span>품목</span><span>특산품 확인 지역</span><span>지역 주소 일치 출원</span><span>그중 등록</span><span>등록 비율</span></div>${visibleRows.map((row, index) => { const expanded = state.expandedItemRegions.has(row.name); return `<div class="item-table-row"><span><b>${String(index + 1).padStart(2, "0")}</b><strong>${esc(row.name)}</strong></span><span class="metric-cell"><button type="button" class="region-count-toggle" data-toggle-item-regions="${esc(row.name)}" aria-expanded="${expanded}"><strong>${row.regions.length}개 지역</strong><small>${expanded ? "접기 ▲" : "지역 보기 ▾"}</small></button>${expanded ? `<small class="region-count-list">${esc(row.regions.join(", "))}</small>` : ""}</span><span class="metric-cell ${row.trademarksDisplay ? "" : "pending-value"}"><strong>${row.trademarksDisplay ? `${number(row.trademarksDisplay)}건${row.hasProvisional ? " (전국 검색 포함)" : ""}` : "집계 대기"}</strong></span><span class="metric-cell ${row.available ? "" : "pending-value"}"><strong>${row.available ? `${number(row.registered)}건` : "집계 대기"}</strong></span><span class="metric-cell ${row.available ? "" : "pending-value"}"><strong>${row.available ? row.trademarks ? percent(row.registered / row.trademarks) : "계산 불가" : "집계 대기"}</strong></span></div>`; }).join("")}</div><div class="method-note"><strong>품목명 집계 기준</strong><p>고시명칭·NICE류가 확정된 품목만 공식 명칭 기준으로 재그룹합니다("풋고추"·"파프리카"는 같은 "고추"로 합쳐짐). 아직 고시명칭이 확정되지 않은 원물명은 이 표에서 제외하고 지역별 상세 화면에 원문 그대로 보존합니다.</p></div></div></section>`;
  }
  function dataScreen() {
  ...
  function compareScreen() {
    return `<section class="screen-section"><div class="screen-heading"><div><p class="eyebrow">SPECIALIZED CROP MATCH</p><h1>특화작목 비교</h1></div><p>정책 지정 작목과 실제 상표 활동의 일치 여부를 비교하는 화면입니다.</p></div><div class="compare-banner"><span>현재 상태</span><strong>정책 지정 특화작목 원본 미수집 · 비교 대기</strong><p>농사로 지역특산물과 정책 지정 특화작목은 같은 데이터가 아니므로 임의로 대체하지 않습니다.</p></div><div class="compare-grid">${[...provinceStats.entries()].sort((a, b) => b[1].trademarksDisplay - a[1].trademarksDisplay).map(([province, row], index) => { const rate = row.decidedItems ? row.appliedItems / row.decidedItems : null; return `<article><div class="compare-head"><h2><b>${String(index + 1).padStart(2, "0")}</b>${esc(province)}</h2><span>비교 대기</span></div><dl><dt>확인 품목 수</dt><dd>${number(row.officialItems)}개${row.decidedItems ? ` · 출원율 ${percent(rate)} (${row.appliedItems}/${row.decidedItems})` : ""}</dd><dt>상표 출원 건수</dt><dd>${row.hasTrademarkData ? `${number(row.trademarksDisplay)}건${row.trademarksDisplay > row.trademarks ? " (전국 검색 포함)" : ""}` : "집계 대기"}</dd><dt>정책 지정 특화작목</dt><dd class="missing">미수집</dd><dt>일치 여부</dt><dd class="missing">판정 불가</dd></dl></article>`; }).join("")}</div><div class="compare-sources"><article><span>추가 수집 1</span><strong>농촌진흥청 지역특화작목 지정 목록</strong><p>계획 기간·지역·작목·근거 버전을 구조화해야 합니다.</p></article><article><span>추가 수집 2</span><strong>한국지식재산연구원 로컬브랜드 근거</strong><p>2024년 보고서의 지역·품목 대응표와 페이지 근거가 필요합니다.</p></article><article><span>자동화 원칙</span><strong>정책 목록 확보 후 결정론적 비교</strong><p>명칭 후보만 개별 검토하고 나머지 집계·일치 판정은 자동화합니다.</p></article></div></section>`;
  }
  ...
  function bind() {
    ...
    document.querySelectorAll("[data-toggle-item-regions]").forEach((button) => { button.onclick = () => { const name = button.dataset.toggleItemRegions; if (state.expandedItemRegions.has(name)) state.expandedItemRegions.delete(name); else state.expandedItemRegions.add(name); render(); }; });
