...
function dashboardClient(snapshot, geometry, registrationExamples) {
  ...
  function provinceDetail(province, regions) {
    ...
    return `<div class="detail-panel province-detail"><div class="detail-heading"><div><p class="eyebrow">광역 기본 보기</p><h2>${esc(displayRegionName(province))}</h2><p>시군구 ${regions.length}곳의 특산품·상표 현황 합계</p></div><span class="state">광역 집계</span></div>${regionTrendHtml({ region: province, items })}<div class="detail-grid province-summary-grid"><article><span>전체 수집 특산품</span><strong>${number(coverage.total)}개</strong><small>시군구별 지역×품목 합계</small></article><article><span>출원 확인 특산품</span><strong>${number(coverage.applied)}개</strong><small>전체 특산품 출원율 ${percent(coverage.rate)}</small></article><article><span>지역 주소 일치 출원</span><strong>${number(applications)}건</strong><small>등록 완료 ${number(registrations)}건</small></article></div><section class="province-municipalities"><div class="section-heading"><div><h2>시군구 상세</h2></div><span>지역을 선택하면 품목별 상세로 전환</span></div><div>${regions.map((region) => { const rowCoverage = specialtyCoverage([region]); const name = region.sigungu && region.sigungu !== region.sido ? region.sigungu : "시도 전체"; return `<button type="button" data-region="${esc(regionKey(region))}"><strong>${esc(name)}</strong><small>특산품 ${rowCoverage.total}개 · 출원 확인 ${rowCoverage.applied}개</small></button>`; }).join("")}</div></section></div>`;
  }
  ...
  function strategyScreen() {
    ...
    const sampleHtml = samples.map(({ region, item }) => `<article class="business-strategy${item.briefing.isGapAlert ? " alert" : ""}"><div class="example-heading"><strong>${esc(region.region)} · ${esc(itemName(item))}</strong><span>${item.briefing.isGapAlert ? "공백 알림" : "양호"}</span></div><ul class="business-strategy-list">${item.briefing.sentences.map((sentence) => `<li>${esc(sentence)}</li>`).join("")}</ul><p class="business-strategy-note">⑤·⑥단계 분석 결과에서 고정 템플릿으로 생성한 문장입니다(${esc(item.briefing.templateVersion || "버전 미기록")}). <button type="button" class="strategy-jump-link" data-open-region="${esc(regionKey(region))}" data-open-item="${esc(item.specialtyId || "")}">지자체별 조회에서 자세히 보기 →</button></p></article>`).join("");
  ...
  function provenanceHtml() {
    ...
    const sourceItems = (sourceId) => ({ admin_codes: "법정동 코드·행정구역명", gi: "농산물 지리적표시", nongsaro: "지역 특산물", nfqs_quality_cert: "인증 수산물(전국)", kofpi_forest_product: "임산물 품목", kipris_trademark: "상표 출원·상태·일자", kipris_trademark_applicant: "출원인 주소", ip_registry: "등록번호·등록일·지정상품", kipo_notice_goods: "고시상품명칭·NICE류", nongsaro_area_brand: "지역 브랜드·출원번호" }[sourceId] || (sourceId.includes("specialties") ? "지역·품목·원문 명칭" : "원천 제공 항목"));
  ...
  function bind() {
    ...
    document.querySelectorAll("[data-kipris-application]").forEach((button) => { button.onclick = () => { window.open(kiprisSearchUrl(button.dataset.kiprisApplication), "kipris-search", "width=1100,height=800,noopener,noreferrer"); }; });
