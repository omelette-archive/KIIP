...
function dashboardClient(snapshot, geometry) {
  ...
  function summaryScreen() {
    ...
    <aside class="map-insight"><p class="eyebrow">SELECTED AREA</p><h2>${esc(state.municipality || state.province || "전국")}</h2><p class="insight-summary">${state.province ? `${visibleRegions.length}개 수집 지역, ${visibleItems.length}개 특산품` : `${snapshot.coverage.observedRegionCount}개 관측 지역 · 검증 완료 항목만 수치 반영`}</p><div class="mini-list">${visibleItems.slice(0, 5).map(({ region, item }) => `<button type="button" data-open-region="${esc(regionKey(region))}" data-open-item="${esc(item.specialtyId || "")}"><span><strong>${esc(region.sigungu || region.region)} / ${esc(officialItemLabel(item))}</strong><small>NICE ${esc(item.niceClass)}류</small></span><b>${item.metrics.uniqueTrademarkCount.availability === "available" ? `출원 ${number(item.metrics.uniqueTrademarkCount.value)}건` : "지역 귀속 검증 중"}</b></button>`).join("") || '<p class="empty">이 지역은 아직 고시명칭이 확정된 품목이 없습니다.</p>'}</div><div class="insight-note"><strong>집계 원칙</strong><p>고시명칭이 확정된 특산물만 표시합니다. 등록 브랜드·상표명은 지역 상세와 "수집된 상표 예시"에서 개별 사례로 확인할 수 있습니다.</p></div></aside></section>
