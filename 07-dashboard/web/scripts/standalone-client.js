...
function dashboardClient(snapshot, geometry, registrationExamples) {
  ...
  const tabs = { summary: "요약", applications: "지역별 상표 출원", regions: "지자체별 조회", items: "품목별 조회", compare: "특화작목 비교", data: "데이터 개요" };
  ...
  const state = { tab: "summary", query: "", itemQuery: "", categoryFilter: "", expandedRegionProvince: null, regionKey: snapshot.regions[0]?.regionCode || snapshot.regions[0]?.region, itemId: "", mapMetric: "coverage", province: null, municipality: null, trendStartYear: null, trendEndYear: null, trendGranularity: "year" };
  ...
  // "YYYY-MM"을 trendScales/trendLinePath가 다루는 연속된 정수 하나로 바꾼다
  ...
  const sumMonthCounts = (items, field) => {
    const totals = {};
    for (const item of items) {
      const counts = item[field];
      if (!counts) continue;
  ...
  const regionTrendHtml = (region) => {
    ...
    if (!years.length) return `<section class="trend-chart trend-chart-compact region-trend"><div class="section-heading"><div><h2>지역 출원·등록 추이</h2></div><span>${esc(region.region)}</span></div><p class="empty">이 지역은 아직 연도별 데이터가 없습니다.</p></section>`;
  ...
  function summaryScreen() {
    ...
    return `<section class="hero"><div class="hero-note"><span>조사 범위</span><strong>${snapshot.coverage.observedRegionCount}개 지역 · ${snapshot.coverage.regionItemCount}건</strong><p>지속 업데이트 예정</p></div></section>
  ...
  function applicationsScreen() {
    ...
    const trendGranularityActive = state.trendGranularity === "month" && trendMonthHasData ? "month" : "year";
    ...
    const trendSvgHtml = trendGranularityActive === "year"
      ? `<svg class="trend-svg" viewBox="0 0 ${TREND_CHART.width} ${TREND_CHART.height}" role="img" aria-label="${trendStart}년부터 ${trendEnd}년까지 연도별 출원·등록 건수 추이">${[0, 0.5, 1].map((fraction) => { const value = Math.round(trendMax * fraction); const yPos = trendScale.y(value); return `<g><line x1="${TREND_CHART.padLeft}" x2="${TREND_CHART.width - TREND_CHART.padRight}" y1="${yPos}" y2="${yPos}" class="trend-gridline" /><text x="${TREND_CHART.padLeft - 8}" y="${yPos}" class="trend-axis-label trend-axis-y">${number(value)}</text></g>`; }).join("")}<path d="${trendLinePath(trendYears, trendApplicationTotals, trendScale)}L${trendScale.x(trendEnd).toFixed(1)},${trendScale.baseY}L${trendScale.x(trendStart).toFixed(1)},${trendScale.baseY}Z" class="trend-area" /><path d="${trendLinePath(trendYears, trendRegisteredTotals, trendScale)}" class="trend-line trend-line-registered" /><path d="${trendLinePath(trendYears, trendApplicationTotals, trendScale)}" class="trend-line trend-line-application" />${trendYears.map((year) => `<circle cx="${trendScale.x(year)}" cy="${trendScale.y(trendApplicationTotals[year] || 0)}" r="2.6" class="trend-point trend-point-application"><title>${year}년 출원 ${number(trendApplicationTotals[year] || 0)}건</title></circle>`).join("")}${trendYears.map((year) => `<circle cx="${trendScale.x(year)}" cy="${trendScale.y(trendRegisteredTotals[year] || 0)}" r="2.6" class="trend-point trend-point-registered"><title>${year}년 등록 ${number(trendRegisteredTotals[year] || 0)}건</title></circle>`).join("")}${trendYearLabels(trendYears).map((year) => `<text x="${trendScale.x(year)}" y="${TREND_CHART.height - 6}" class="trend-axis-label trend-axis-x">${year}</text>`).join("")}</svg>`
      : `<svg class="trend-svg" viewBox="0 0 ${TREND_CHART.width} ${TREND_CHART.height}" role="img" aria-label="${trendStart}년 ${trendEnd}년 월별 출원·등록 건수 추이">${[0, 0.5, 1].map((fraction) => { const value = Math.round(trendMonthMax * fraction); const yPos = trendMonthScale.y(value); return `<g><line x1="${TREND_CHART.padLeft}" x2="${TREND_CHART.width - TREND_CHART.padRight}" y1="${yPos}" y2="${yPos}" class="trend-gridline" /><text x="${TREND_CHART.padLeft - 8}" y="${yPos}" class="trend-axis-label trend-axis-y">${number(value)}</text></g>`; }).join("")}<path d="${trendLinePath(trendMonthIndexes, trendMonthApplicationTotals, trendMonthScale)}L${trendMonthScale.x(trendMonthIndexes[trendMonthIndexes.length - 1]).toFixed(1)},${trendMonthScale.baseY}L${trendMonthScale.x(trendMonthIndexes[0]).toFixed(1)},${trendMonthScale.baseY}Z" class="trend-area" /><path d="${trendLinePath(trendMonthIndexes, trendMonthRegisteredTotals, trendMonthScale)}" class="trend-line trend-line-registered" /><path d="${trendLinePath(trendMonthIndexes, trendMonthApplicationTotals, trendMonthScale)}" class="trend-line trend-line-application" />${trendMonthIndexes.map((index) => `<circle cx="${trendMonthScale.x(index)}" cy="${trendMonthScale.y(trendMonthApplicationTotals[index] || 0)}" r="2.2" class="trend-point trend-point-application"><title>${monthKeyFromIndex(index)} 출원 ${number(trendMonthApplicationTotals[index] || 0)}건</title></circle>`).join("")}${trendMonthIndexes.map((index) => `<circle cx="${trendMonthScale.x(index)}" cy="${trendMonthScale.y(trendMonthRegisteredTotals[index] || 0)}" r="2.2" class="trend-point trend-point-registered"><title>${monthKeyFromIndex(index)} 등록 ${number(trendMonthRegisteredTotals[index] || 0)}건</title></circle>`).join("")}${trendYearLabels(trendMonthIndexes).map((index) => `<text x="${trendMonthScale.x(index)}" y="${TREND_CHART.height - 6}" class="trend-axis-label trend-axis-x">${esc(monthKeyFromIndex(index))}</text>`).join("")}</svg>`;
    const trendGranularityToggleHtml = trendMonthHasData
      ? `<div class="trend-granularity" role="group" aria-label="추이 그래프 단위"><button type="button" data-trend-granularity="year" class="${trendGranularityActive === "year" ? "active" : ""}">연도별</button><button type="button" data-trend-granularity="month" class="${trendGranularityActive === "month" ? "active" : ""}">월별</button></div>`
      : "";
    const trendCoverageNoteHtml = trendGranularityActive === "month" && trendItemsWithMonthData.length < trendItemsWithYearData.length
      ? `<p class="trend-coverage-note">월별 데이터는 이 범위 품목 ${trendItemsWithYearData.length}개 중 ${trendItemsWithMonthData.length}개에만 있습니다(나머지는 연도별로만 조회 가능).</p>`
      : "";
    const trendChartHtml = trendHasData
      ? `<div class="trend-controls"><div class="trend-presets" role="group" aria-label="추이 그래프 기간 프리셋"><button type="button" data-trend-preset="all" class="${trendStart === trendFullStart && trendEnd === trendFullEnd ? "active" : ""}">전체</button><button type="button" data-trend-preset="5" data-trend-full-end="${trendFullEnd}" class="${trendStart === trendFullEnd - 4 && trendEnd === trendFullEnd ? "active" : ""}">최근 5년</button><button type="button" data-trend-preset="3" data-trend-full-end="${trendFullEnd}" class="${trendStart === trendFullEnd - 2 && trendEnd === trendFullEnd ? "active" : ""}">최근 3년</button><button type="button" data-trend-preset="1" data-trend-full-end="${trendFullEnd}" class="${trendStart === trendFullEnd && trendEnd === trendFullEnd ? "active" : ""}">최근 1년</button></div><div class="trend-range-inputs"><label><span class="sr-only">시작 연도</span>${trendStart}<input type="number" id="trend-start-input" aria-label="시작 연도" value="${trendStart}"></label><span>~</span><label><span class="sr-only">끝 연도</span>${trendEnd}<input type="number" id="trend-end-input" aria-label="끝 연도" value="${trendEnd}"></label></div>${trendGranularityToggleHtml}</div>${trendCoverageNoteHtml}<div class="trend-range-slider"><span class="trend-range-label">${trendFullStart}년 – ${trendFullEnd}년 중 ${trendStart}년 – ${trendEnd}년 선택</span><div class="trend-range-track" data-full-start="${trendFullStart}" data-full-end="${trendFullEnd}"><div class="trend-range-fill" style="left:${trendHandlePercent(trendStart, trendFullStart, trendFullEnd)}%;right:${100 - trendHandlePercent(trendEnd, trendFullStart, trendFullEnd)}%"></div><button type="button" id="trend-range-handle-start" class="trend-range-handle trend-range-handle-start" role="slider" aria-label="시작 연도 조절" aria-valuemin="${trendFullStart}" aria-valuemax="${trendEnd}" aria-valuenow="${trendStart}" data-value="${trendStart}" style="left:${trendHandlePercent(trendStart, trendFullStart, trendFullEnd)}%"></button><button type="button" id="trend-range-handle-end" class="trend-range-handle trend-range-handle-end" role="slider" aria-label="끝 연도 조절" aria-valuemin="${trendStart}" aria-valuemax="${trendFullEnd}" aria-valuenow="${trendEnd}" data-value="${trendEnd}" style="left:${trendHandlePercent(trendEnd, trendFullStart, trendFullEnd)}%"></button></div></div>${trendSvgHtml}<p class="trend-legend"><span class="trend-legend-swatch trend-legend-application"></span>출원<span class="trend-legend-swatch trend-legend-registered"></span>등록(등록원부 보강 완료 건)</p>`
  ...
  function bind() {
    ...
    document.querySelectorAll("[data-trend-granularity]").forEach((button) => { button.onclick = () => { state.trendGranularity = button.dataset.trendGranularity; render(); }; });
