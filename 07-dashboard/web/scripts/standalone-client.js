...
function dashboardClient(snapshot, geometry) {
  ...
  function summaryScreen() {
    ...
    return `<section class="hero"><div><p class="eyebrow">LOCAL BRAND OBSERVATORY</p><h1>지역 특산품 상표 분석</h1><p class="hero-copy">지역별 특산품과 관련 상표 현황을 한눈에 확인합니다.</p></div><div class="hero-note"><span>DATA COVERAGE</span><strong>${snapshot.coverage.observedRegionCount}개 지역 · ${snapshot.coverage.regionItemCount}개 지역×품목</strong><p>${isAlpha && pipeline ? `주소 확보율 ${percent(pipeline.applicantRegionVerification.rate)} · 확보된 값 기준으로 표시합니다.` : "현재 확인 가능한 데이터 범위입니다."}</p></div></section>
    ...
    <aside class="map-insight"><p class="eyebrow">SELECTED AREA</p><h2>${esc(state.municipality || state.province || "전국")}</h2><p class="insight-summary">${state.province ? `${visibleRegions.length}개 수집 지역, ${visibleItems.length}개 특산품` : `${snapshot.coverage.observedRegionCount}개 관측 지역 · 검증 완료 항목만 수치 반영`}</p><div class="mini-list">${visibleItems.slice(0, 5).map(({ region, item }) => `<button type="button" data-open-region="${esc(regionKey(region))}" data-open-item="${esc(item.specialtyId || "")}"><span><strong>${esc(region.sigungu || region.region)} / ${esc(itemName(item))}</strong><small>${esc(noticeBasis(item))} · NICE ${esc(item.niceClass || "미확정")}류</small></span><b>${item.metrics.uniqueTrademarkCount.availability === "available" ? `출원 ${number(item.metrics.uniqueTrademarkCount.value)}건` : "지역 귀속 검증 중"}</b></button>`).join("") || '<p class="empty">이 지역의 수집 데이터가 없습니다.</p>'}</div><div class="insight-note"><strong>집계 원칙</strong><p>대표 특산품은 표준명으로 표시합니다. 상표 수치는 확보된 출원인 주소 기준이며, 미확보 건은 원자료와 함께 참고값으로 남깁니다.</p></div></aside></section>
