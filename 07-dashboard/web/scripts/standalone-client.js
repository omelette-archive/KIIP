...
function dashboardClient(snapshot, geometry) {
  ...
  function summaryScreen() {
    ...
    <section class="metrics"><article><span>지역 고유 상표</span><strong>${totals.availableItems ? number(totals.trademarks) : "검증 중"}</strong><small>출원인 주소 inside 확정 건만</small></article><article><span>지역 등록 상표</span><strong>${totals.availableItems ? number(totals.registered) : "검증 중"}</strong><small>지역 귀속 완료 항목만 합산</small></article><article><span>지정상품 검토 후보</span><strong>${number(totals.review)}</strong><small>확실한 항목만 자동 확정</small></article><article><span>수집 상태</span><strong>${snapshot.coverage.partialQueryCount ? "부분" : "완료"}</strong><small>완료 ${snapshot.coverage.completeQueryCount} · 부분 ${snapshot.coverage.partialQueryCount}</small></article></section>
  ...
  function criteria() {
    const rows = [
      ...
      ["지역 상표 / 등록 상표", "출원인 주소 inside 검증 건만", "전국 KIPRIS 검색 후보는 별도 보존하며 지역 건수·등록률에 포함하지 않음"],
      ["출원인 지역 매칭", "전체 주소 귀속 완료 전 지표 차단", "검색 수집 또는 주소 검증이 일부뿐이면 건수·공백 점수를 0이 아닌 검증 중으로 표시"],
    ];
