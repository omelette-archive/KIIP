...
function dashboardClient(snapshot, geometry) {
  ...
  // 2026-08-19 방향 전환: 지역 확인이 안 끝난 상표를 전국 검색 건수로 대신 보여주던
  // 방식(잠정치/전국 검색 표시)을 걷어낸다. 전국 키워드 검색은 지역과 무관한 값이라
  // 지역 수치처럼 보여주면 실제보다 부풀려 보인다는 지적에 따라, 대시보드는 다시
  // 지역 귀속이 확정된 값만 보여주고, 확정되지 않은 항목은 "집계 대기"로 정직하게
  // 표시한다. 남은 항목을 더 많이 확정하는 일은 ③ 상표 매칭 단계의 --enrich-registry
  // 주소 보강(등록원부 조회)을 더 진행하는 데이터 파이프라인 작업으로 처리한다.
  ...
  function compareScreen() {
    return `<section class="screen-section"><div class="screen-heading"><div><p class="eyebrow">SPECIALIZED CROP MATCH</p><h1>특화작목 비교</h1></div><p>정책 지정 작목과 실제 상표 활동의 일치 여부를 비교하는 화면입니다.</p></div><div class="compare-banner"><span>현재 상태</span><strong>정책 지정 특화작목 원본 미수집 · 비교 대기</strong><p>농사로 지역특산물과 정책 지정 특화작목은 같은 데이터가 아니므로 임의로 대체하지 않습니다.</p></div><div class="compare-grid">${[...provinceStats.entries()].sort((a, b) => b[1].trademarks - a[1].trademarks).map(([province, row], index) => { const rate = row.decidedItems ? row.appliedItems / row.decidedItems : null; return `<article><div class="compare-head"><h2><b>${String(index + 1).padStart(2, "0")}</b>${esc(province)}</h2><span>비교 대기</span></div><dl><dt>확인 품목 수</dt><dd>${number(row.officialItems)}개${row.decidedItems ? ` · 출원율 ${percent(rate)} (${row.appliedItems}/${row.decidedItems})` : ""}</dd><dt>상표 출원 건수</dt><dd>${row.verified ? `${number(row.trademarks)}건` : "집계 대기"}</dd><dt>정책 지정 특화작목</dt><dd class="missing">미수집</dd><dt>일치 여부</dt><dd class="missing">판정 불가</dd></dl></article>`; }).join("")}</div><div class="compare-sources"><article><span>추가 수집 1</span><strong>농촌진흥청 지역특화작목 지정 목록</strong><p>계획 기간·지역·작목·근거 버전을 구조화해야 합니다.</p></article><article><span>추가 수집 2</span><strong>한국지식재산연구원 로컬브랜드 근거</strong><p>2024년 보고서의 지역·품목 대응표와 페이지 근거가 필요합니다.</p></article><article><span>자동화 원칙</span><strong>정책 목록 확보 후 결정론적 비교</strong><p>명칭 후보만 개별 검토하고 나머지 집계·일치 판정은 자동화합니다.</p></article></div></section>`;
  }
