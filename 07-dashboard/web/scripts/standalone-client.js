...
function dashboardClient(snapshot, geometry) {
  ...
  // "구분 정보 없음"으로 표시한다.
  function specialtyFilingStatus(item, region) {
    if (item.metrics.uniqueTrademarkCount.availability !== "available") {
      return { label: "검토중", className: "pending" };
    }
    const value = item.metrics.uniqueTrademarkCount.value || 0;
    if (value > 0) return { label: `출원 확인 · ${number(value)}건`, className: "filed" };
    if (!region.regionCode) return { label: "구분 정보 없음", className: "pending" };
    return { label: "출원 미확인", className: "unfiled" };
  }
