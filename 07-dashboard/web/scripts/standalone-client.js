...
function dashboardClient(snapshot, geometry) {
  ...
  const mapLabels = { trademarks: "상표 건수", registration: "상표 등록률", coverage: "확인 특산품 수", gap: "상표 활용 여지" };
  ...
  snapshot.regions.forEach((region) => {
    ...
    const row = provinceStats.get(name) || { trademarks: 0, registered: 0, verified: 0, items: 0, complete: 0, gaps: [] };
  ...
  function itemRows() {
    ...
    snapshot.regions.forEach((region) => region.items.forEach((item) => {
      ...
      const row = rows.get(name) || { name, trademarks: 0, registered: 0, available: 0, regions: [] };
