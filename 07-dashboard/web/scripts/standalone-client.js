...
function dashboardClient(snapshot, geometry, registrationExamples) {
  ...
  const PRIMARY_NAV = [["summary", "요약"], ["applications", "지역별 특산품 상표 현황"], ["items", "품목별 특산품 상표 현황"], ["strategy", "비즈니스 확장 경로 분석"], ["compare", "특화작목 비교"], ["data", "데이터 개요"]];
  ...
  // #175: 급증 카드 가속/신규진입 분리 임계값(1.3배는 4개 창 전부 견고, 교차검증).
  const LEADER_ACCEL_MIN_GROWTH = 1.3;
  const LEADER_FRESH_LIMIT = 3;
  // #119: 비즈니스 전략 탭 주요 품목 칩 개수. 나머지는 수기 검색(협업자 요청 2026-09-02).
  const STRATEGY_CHIP_LIMIT = 12;
  // #119: 육성 vs 출원 활발 3갈래 목록 각 최대 개수.
  ...
  // 정규화해 각을 나누고 원래 점유율은 범례에 그대로 적는다. Dashboard.tsx StageRegionDonut과 동일.
  ...
  const nationwideFlowCardHtml = (flow, itemLabel, origins) => {
    ...
    const regionDonutsHtml = ["raw", "processed", "service"].some((key) => (flow.stages[key].topRegions || []).length > 0)
  ...
  function computeLeaderboard() {
    ...
    // #175: 배수 하나로 정렬하면 직전 0~1건 품목이 무한 배수로 독점 → 가속/신규 진입 분리.
  ...
  function leaderboardHtml() {
    ...
    const rangeNote = `<p class="leader-window-range">${esc(monthRangeLabel(lb.windowKeys))} 집계 · 급증은 직전 ${esc(monthRangeLabel(lb.priorKeys))}과 비교</p>`;
    ...
    const itemsCard = `<article class="leader-card"><div class="leader-card-head"><h4>${metricWord} 많은 품목</h4><span class="leader-card-note">TOP ${LEADER_LIMIT}</span></div>${lb.topItems.length === 0 ? '<p class="empty">해당 기간 집계가 없습니다.</p>' : `<ol class="leader-list">${lb.topItems.map((row, index) => { const value = topValue(row); return `<li><button type="button" data-goto-item="${esc(row.name)}"><span class="leader-rank">${index + 1}</span><span class="leader-name">${esc(row.name)}${tag(row.category)}</span><b class="leader-val">${number(value)}</b><span class="leader-bar"><i style="width:${topMax ? Math.max(3, value / topMax * 100) : 0}%"></i></span></button></li>`; }).join("")}</ol>`}</article>`;
    const catsCard = `<article class="leader-card"><div class="leader-card-head"><h4>${metricWord} 많은 유형</h4><span class="leader-card-note">비중</span></div>${lb.topCategories.length === 0 ? '<p class="empty">해당 기간 집계가 없습니다.</p>' : `<ol class="leader-list">${lb.topCategories.slice(0, LEADER_LIMIT).map((row, index) => { const value = state.leaderMetric === "application" ? row.app : row.reg; const share = lb.categoryTotal ? value / lb.categoryTotal : 0; return `<li><button type="button" title="비중 ${percent(share)}" data-goto-category="${esc(row.code || "")}"><span class="leader-rank">${index + 1}</span><span class="leader-name">${esc(row.label)}</span><b class="leader-val">${number(value)}</b><span class="leader-bar"><i style="width:${Math.max(3, share * 100)}%;background:${categoryShareColor(row.label)}"></i></span></button></li>`; }).join("")}</ol>`}</article>`;
    const accelMax = lb.accelerating.length ? lb.accelerating[0].growth : 0;
    const accelHtml = lb.accelerating.length === 0
      ? `<p class="empty">직전 기간 대비 ${LEADER_ACCEL_MIN_GROWTH}배 이상 성장한 품목이 없습니다.</p>`
      : `<ol class="leader-list">${lb.accelerating.map((row, index) => `<li><button type="button" title="직전 ${number(row.priorApp)}건 → 최근 ${number(row.app)}건 (+${number(row.delta)})" data-goto-item="${esc(row.name)}"><span class="leader-rank">${index + 1}</span><span class="leader-name">${esc(row.name)}${tag(row.category)}</span><b class="leader-val">×${row.growth >= 10 ? Math.round(row.growth) : row.growth.toFixed(1)}</b><span class="leader-bar"><i style="width:${accelMax ? Math.max(3, row.growth / accelMax * 100) : 0}%"></i></span></button></li>`).join("")}</ol>`;
    const freshHtml = lb.freshEntries.length === 0 ? "" : `<div class="leader-inline-head"><span>신규 진입</span><small>직전 기간 거의 없던 품목</small></div><ol class="leader-list">${lb.freshEntries.map((row, index) => `<li><button type="button" title="직전 ${number(row.priorApp)}건 → 최근 ${number(row.app)}건" data-goto-item="${esc(row.name)}"><span class="leader-rank">${index + 1}</span><span class="leader-name">${esc(row.name)}${tag(row.category)}</span><b class="leader-val leader-val-wide">${number(row.priorApp)}→${number(row.app)}</b></button></li>`).join("")}</ol>`;
    const surgeCard = `<article class="leader-card"><div class="leader-card-head"><h4>출원 급증 품목</h4><span class="leader-card-note">직전 기간 대비</span></div>${lb.accelerating.length === 0 && lb.freshEntries.length === 0 ? `<p class="empty">뚜렷한 급증 품목이 없습니다(최소 출원 ${lb.minBase}건).</p>` : `${accelHtml}${freshHtml}`}</article>`;
    const convRow = (row, index, low) => `<li><button type="button" title="등록 ${number(row.lifeReg)}건 / 출원 ${number(row.lifeApp)}건" data-goto-item="${esc(row.name)}"><span class="leader-rank">${index + 1}</span><span class="leader-name">${esc(row.name)}${tag(row.category)}</span><b class="leader-val">${percent(row.rate)}</b><span class="leader-bar${low ? " leader-bar-low" : ""}"><i style="width:${Math.max(3, row.rate * 100)}%"></i></span></button></li>`;
  ...
  const regionTrademarkPanelHtml = (row) => {
    ...
    const seen = new Set();
    const examples = picked.flatMap((entry) => entry.item.trademarkExamples || []).filter((example) => {
    ...
    const listHtml = examples.length
      ...
      : `<p class="empty">이 지역의 상표 예시가 아직 수집되지 않았습니다.</p>`;
  ...
  function itemDetailHtml(row) {
    ...
    const chips = [...row.regions].sort((a, b) => (row.regionCounts[b] || 0) - (row.regionCounts[a] || 0)).map((region) => { const value = row.regionCounts[region] || 0; const max = Math.max(1, ...Object.values(row.regionCounts)); return `<button type="button" data-item-region="${esc(region)}" class="region-chip-button${state.itemRegionPick === region ? " active" : ""}" style="font-size:${wordCloudFontSize(value, max)}px;color:${wordCloudColor(region)}" title="${esc(displayRegionName(region))} · 출원 ${number(value)}건">${esc(displayRegionName(region))}</button>`; }).join("");
    ...
    const flowLb = flowItem ? computeLeaderboard() : null;
    const flowSurging = flowLb ? flowLb.accelerating.some((s) => s.name === row.name) || flowLb.freshEntries.some((s) => s.name === row.name) : false;
    const flowHtml = flowItem ? nationwideFlowCardHtml(flowItem.businessFlow, row.name, [...row.regions].sort((a, b) => (row.regionCounts[b] || 0) - (row.regionCounts[a] || 0)).slice(0, 3)) + expansionSuggestionsHtml(flowItem.businessFlow, row.name, flowSurging) : "";
  ...
  function itemsScreen() {
    ...
    return `<li><button type="button" data-select-item="${esc(row.name)}" class="${selected && selected.name === row.name ? "active" : ""}" title="${title}"><span class="item-list-name">${esc(row.name)}</span>${sparkHtml}<b>${decidedRegions ? `${number(row.trademarks)}건` : "집계 대기"}</b></button></li>`;
  ...
  function bindSearchInput(selector, stateKey) {
    ...
    const commit = (value) => {
      ...
      // #119: 품목 직접 검색을 바꾸면 칩 선택을 풀고 첫 검색 결과를 보여준다(React와 동일).
      if (stateKey === "strategyItemQuery") state.strategyItem = "";
      render();
  ...
  function strategyFlowHtml() {
    ...
    const lb = computeLeaderboard();
    const surging = lb.accelerating.some((s) => s.name === selected.name) || lb.freshEntries.some((s) => s.name === selected.name);
  ...
  function strategyScreen() {
    ...
    if (rows.length === 0) return `<section class="screen-section strategy-screen">${flowHtml}<p class="empty">아직 표시할 지역×품목 브리핑이 없습니다.</p></section>`;
    ...
    return `<section class="screen-section strategy-screen">${flowHtml}<details class="strategy-table-toggle"><summary><span>지역×품목 공백 알림 표 전체 보기</span><small>${rows.length}건</small></summary><p class="screen-note">지역×품목 단위 비즈니스 확장 전략 브리핑입니다. 표에서 행을 고르면 오른쪽에 브리핑과 근거가 열립니다. <b>지역 확인 출원</b>은 출원인 주소가 그 지역과 일치한 확정 건수, <b>전국 검색</b>은 같은 품목명의 전국 검색 결과(지역 확인 전 후보)로 서로 다른 모집단이며, <b>전국 대비</b>는 두 값의 비율입니다.</p>${toolbarHtml}<div class="strategy-table-layout">${tableHtml}<aside class="strategy-detail">${detailHtml}</aside></div></details></section>`;
  }
  // 이슈 #119(협업자 2026-09-02): 육성 작목 vs 출원 활발 품목 3갈래 대조. Dashboard.tsx와 동일.
  function matchBlockHtml(row) {
    if (!row) return "";
    const cropLi = (crop, count) => `<li><span>${esc(crop.displayName)}<em class="crop-badge crop-badge-${esc(crop.tier)}">${esc(crop.tier)}</em></span><b>${count}</b></li>`;
    const activeHtml = row.policyActive.length === 0 ? '<p class="empty">해당 작목이 없습니다.</p>' : `<ol class="compare-match-list">${row.policyActive.map((c) => cropLi(c, number(c.applications) + "건")).join("")}</ol>`;
    const dormantHtml = row.policyDormant.length === 0 ? '<p class="empty">집계 완료 작목은 모두 출원이 확인됐습니다.</p>' : `<ol class="compare-match-list">${row.policyDormant.map((c) => cropLi(c, "0건")).join("")}</ol>`;
    const unlistedHtml = row.unlistedActive.length === 0 ? '<p class="empty">육성 작목 밖에서 출원이 확인된 품목이 없습니다.</p>' : `<ol class="compare-match-list">${row.unlistedActive.map((r) => `<li><span>${esc(r.displayName)}</span><b>${number(r.count)}건</b></li>`).join("")}</ol>`;
    return `<section class="compare-match-block"><div class="section-heading"><div><h3>육성 작목 vs 상표 출원이 활발한 품목</h3></div><span>${esc(displayRegionName(row.province))} · 지역 주소 일치 출원 기준</span></div><div class="compare-match-grid"><article class="compare-match-card match"><div class="compare-match-head"><h4>육성 + 출원 활발</h4><span>정책·시장 일치</span></div>${activeHtml}</article><article class="compare-match-card dormant"><div class="compare-match-head"><h4>육성 중, 출원 없음</h4><span>브랜드화 지원 검토</span></div>${dormantHtml}</article><article class="compare-match-card unlisted"><div class="compare-match-head"><h4>출원 활발, 육성 대상 아님</h4><span>지정 검토 대상</span></div>${unlistedHtml}</article></div><p class="compare-match-note">농촌진흥청 특화작목(대표·집중육성·자체육성) 지정 여부와 실제 상표 출원을 대조합니다. 세 번째 칸은 이 도에서 상표 출원은 활발한데 특화작목으로 지정되지 않은 품목으로, 지정 범위 재검토 신호로 볼 수 있습니다.</p></section>`;
  }
  function compareScreen() {
    const comparisonRows = [...provinceStats.keys()].map((province) => {
      ...
      // 이슈 #119: 육성 작목 전체 vs 출원 활발 품목 3갈래 대조. Dashboard.tsx와 동일.
      const appliedByName = new Map();
      for (const region of regions) for (const item of region.items) {
        const name = officialItemLabel(item);
        if (!name || item.metrics.uniqueTrademarkCount.availability !== "available") continue;
      ...
      // 원문·괄호 제거형을 모두 제외 집합에 넣어야 같은 품목이 양쪽 칸에 겹치지 않는다.
      ...
      const unlistedActive = [...appliedByName.entries()].filter(([name, count]) => count > 0 && !policyNames.has(name) && !policyNames.has(stripParens(name))).sort((a, b) => b[1] - a[1]).slice(0, COMPARE_MATCH_LIMIT).map(([name, count]) => ({ name, displayName: stripParens(name), count }));
      return { province, policyCrops, policyApplicationsTotal, policyDecided, policyApplied, policyRate: policyCrops.length ? policyApplied / policyCrops.length : null, topRegisteredItems, flagshipCrop, flagshipMatch, flagshipRank, policyActive, policyDormant, unlistedActive };
