...
const [snapshotText, geometryText, cssText, client, logoMarkBuffer, logoLockupBuffer] = await Promise.all([
...
<body><main class="shell"><header class="topbar" id="top"><button class="brand brand-button" id="brand-home" type="button"><img class="brand-mark" src="${logoMarkDataUri}" alt="KIIP" width="36" height="24"><span><strong>지역 브랜드 인사이트</strong><small>지역 특산품 상표 현황</small></span></button><div class="snapshot-meta"><span class="sample-badge" id="scope-label"></span><span id="generated"></span></div></header><nav class="primary-tabs" id="primary-tabs" aria-label="대시보드 화면"></nav><div id="app"></div><footer><img class="footer-logo" src="${logoLockupDataUri}" alt="한국지식재산연구원 Korea Institute of Intellectual Property" height="26"><span id="snapshot-id"></span></footer></main>
