...
export default function Dashboard({ snapshot, geometry }: { snapshot: Snapshot; geometry: MapGeometry }) {
  ...
  }), [provinceStats, snapshot.regions]);
  ...
  return <main className="shell">
    ...
    {tab === "items" && <section className="screen-section">
      ...
      <div className="item-screen">
        <div className="item-screen-toolbar"><label><span className="sr-only">품목 검색</span><input value={itemQuery} onChange={(event) => setItemQuery(event.target.value)} placeholder="품목명 또는 지역명 검색" /></label><span>검색 결과 {itemRows.length}개</span></div>
        <div className="item-reading-guide"><strong>수치 구분</strong><span><b>지역 확인 출원</b> 출원인 주소가 해당 지역과 일치</span><span><b>잠정 후보</b> 지역 확인 전 전국 검색 결과</span></div>
        <div className="item-card-grid">{itemRows.map((row, index) => { const decidedRegions = row.availableRegions.length; const pendingRegions = Math.max(0, row.regions.length - decidedRegions); const provisional = Math.max(0, row.trademarksDisplay - row.trademarks); return <article className="item-card" key={row.name}>
          ...
          <div className="region-chips">{row.regions.slice(0, 3).map((region) => <span key={region}>{region}</span>)}{row.regions.length > 3 && <span>외 {row.regions.length - 3}곳</span>}</div>
          ...
          {provisional > 0 && <p className="provisional-note">지역 미확정 전국 검색 후보 {number(provisional)}건은 위 확정 수치에 포함하지 않았습니다.</p>}
    ...
    {tab === "compare" && <section className="screen-section">
      ...
      <div className="compare-sources"><article><span>필수 입력</span><strong>농촌진흥청 지역특화작목 지정 목록</strong><p>지역·작목·계획 기간·근거 버전을 구조화해야 합니다.</p></article><article><span>처리 원칙</span><strong>원본 확보 후 자동 비교</strong><p>명칭 정규화 후보만 사람이 검토하고 집계·일치 판정은 자동화합니다.</p></article></div>
    </section>}
