  ...
  return <main className="shell">
    ...
    {tab === "applications" && <section className="screen-section coverage-screen">
      ...
      <section className="coverage-directory"><div className="section-heading"><div><h2>{coverageAreaDisplayName} 특산품 목록</h2></div><span>확인 {number(coverageArea.total)}개 · 출원 확인 {number(coverageArea.applied)}개 · 출원율 {percent(coverageArea.rate)}</span></div>
        <div className="coverage-region-grid">{coverageBreakdown.map((row) => <article className={selectedMunicipality && row.label === selectedMunicipality ? "coverage-region-card selected" : "coverage-region-card"} key={row.key}><div className="coverage-region-head"><div><strong>{displayRegionName(row.label)}</strong><small>특산품 {number(row.coverage.total)}개 · 출원 확인 {number(row.coverage.applied)} / 판정 완료 {number(row.coverage.decided)}{row.coverage.pending ? ` · 대기 ${number(row.coverage.pending)}` : ""}</small></div><b>{percent(row.coverage.rate)}</b>{!selectedProvince && <button type="button" onClick={() => openProvince(row.label)}>지도에서 보기</button>}</div><div className="coverage-specialty-list">{row.items.map(({ region, item, label }) => <button type="button" key={`${regionKey(region)}-${item.specialtyId}`} onClick={() => { chooseRegion(region); setSelectedItemId(item.specialtyId || ""); setTab("regions"); }}><span>{selectedProvince ? label : `${region.sigungu || region.region} / ${label}`}</span><small>{item.metrics.uniqueTrademarkCount.availability === "available" ? (item.metrics.uniqueTrademarkCount.value || 0) > 0 ? `출원 확인 · ${number(item.metrics.uniqueTrademarkCount.value)}건` : "출원 없음 · 판정 완료" : "집계 대기"}</small></button>)}</div></article>)}</div>
      </section>
    </section>}
