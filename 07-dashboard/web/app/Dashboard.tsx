...
type MapMetric = "trademarks" | "registration" | "coverage" | "filing";
...
const MAP_LABELS: Record<MapMetric, string> = { trademarks: "상표 건수", registration: "등록률", coverage: "수집 범위", filing: "출원율" };
...
export default function Dashboard({ snapshot, geometry }: { snapshot: Snapshot; geometry: MapGeometry }) {
  ...
  const provinceStats = useMemo(() => {
    const stats = new Map<string, { trademarks: number; registered: number; verified: number; items: number; complete: number; nonzero: number; total: number }>();
    snapshot.regions.forEach((region) => {
      const name = region.sido || region.region;
      const current = stats.get(name) || { trademarks: 0, registered: 0, verified: 0, items: 0, complete: 0, nonzero: 0, total: 0 };
      region.items.forEach((item) => { if (item.metrics.uniqueTrademarkCount.availability === "available") { current.verified += 1; current.trademarks += item.metrics.uniqueTrademarkCount.value || 0; current.registered += item.metrics.registeredTrademarkCount.value || 0; } current.items += 1; current.total += 1; if (item.dataState === "complete_nonzero" || item.dataState === "complete_zero") { current.complete += 1; if (item.dataState === "complete_nonzero") current.nonzero += 1; } });
      stats.set(name, current);
    });
    return stats;
  }, [snapshot.regions]);
  const mapMax = Math.max(1, ...[...provinceStats.values()].map((stat) => mapMetric === "trademarks" ? stat.trademarks : mapMetric === "registration" ? (stat.trademarks ? stat.registered / stat.trademarks : 0) : mapMetric === "coverage" ? stat.items : (stat.complete ? stat.nonzero / stat.complete : 0)));
  ...
  const itemRows = useMemo(() => {
    const rows = new Map<string, { name: string; trademarks: number; registered: number; verified: number; regions: string[]; states: string[] }>();
    snapshot.regions.forEach((region) => region.items.forEach((item) => {
      ...
      const row = rows.get(name) || { name, trademarks: 0, registered: 0, verified: 0, regions: [], states: [] };
  ...
  function regionFilingSummary(region: Region | undefined) {
  ...
  function municipalityValue(region: Region | undefined) {
    if (!region) return null;
    if (mapMetric === "coverage") return region.items.length;
    if (mapMetric === "filing") return regionFilingSummary(region)?.rate ?? null;
    const trademarks = regionTrademarkValue(region);
    if (mapMetric === "trademarks") return trademarks;
    if (trademarks === null) return null;
    const registered = region.items.filter((item) => item.metrics.registeredTrademarkCount.availability === "available").reduce((sum, item) => sum + (item.metrics.registeredTrademarkCount.value || 0), 0);
    return trademarks ? registered / trademarks : 0;
  }
  function municipalityValueLabel(value: number | null) { if (value === null) return "데이터 검토 중"; if (mapMetric === "registration" || mapMetric === "filing") return percent(value); return `${number(value)}${mapMetric === "trademarks" ? "건" : "개 품목"}`; }
  function mapValue(name: string) { const stat = provinceStats.get(name); if (!stat) return null; if (mapMetric === "trademarks") return stat.verified ? stat.trademarks : null; if (mapMetric === "registration") return stat.verified && stat.trademarks ? stat.registered / stat.trademarks : null; if (mapMetric === "coverage") return stat.items; return stat.complete ? stat.nonzero / stat.complete : null; }
  function mapValueLabel(name: string) { const value = mapValue(name); if (value === null) return "데이터 없음"; if (mapMetric === "registration" || mapMetric === "filing") return percent(value); return `${number(value)}${mapMetric === "trademarks" ? "건" : "개 품목"}`; }
  ...
  const municipalityMapMax = municipalityGeometry ? Math.max(1, ...municipalityGeometry.items.map((shape) => municipalityValue(snapshot.regions.find((region) => region.sido === selectedProvince && region.sigungu === shape.name)) || 0)) : 1;
  ...
  return <main className="shell">
      ...
      <section className="map-workspace">
        <div className="map-card"><div className="map-heading"><div><p className="eyebrow">BRAND GAP MAP</p><h2>{selectedProvince ? `${selectedProvince} 시군구` : "전국 지역 브랜드 지도"}</h2></div><span className="reference-chip">참고 경계 · 2013 KOSTAT</span></div>
          ...
          <div className="map-stage"><svg className="korea-map" viewBox={municipalityGeometry?.viewBox || geometry.viewBox} role="img" aria-label={selectedProvince ? `${selectedProvince} 시군구 지도` : "대한민국 시도 지도"}>{municipalityGeometry ? <>
            {municipalityGeometry.items.map((shape) => { const match = snapshot.regions.find((region) => region.sido === selectedProvince && region.sigungu === shape.name); const statValue = municipalityValue(match); const statLabel = municipalityValueLabel(statValue); const active = selectedMunicipality === shape.name; return <path key={`${shape.name}-shape`} d={shape.d} className={active ? "map-shape selected" : "map-shape"} style={{ fill: fill(statValue, municipalityMapMax) }} tabIndex={0} role="button" aria-label={`${shape.name} ${statLabel}`} onClick={() => openMunicipality(shape.name)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") openMunicipality(shape.name); }}><title>{shape.name} · {statLabel}</title></path>; })}
    ...
    {tab === "regions" && <section className="screen-section"><div className="screen-heading"><div><p className="eyebrow">LOCAL GOVERNMENT</p><h1>지자체별 조회</h1></div><p>지역 → 품목 → 근거 지표 순으로 확인합니다.</p></div><section className="workspace" aria-label="지역별 상세 조회"><aside className="region-panel"><div className="panel-heading"><div><p className="eyebrow">REGION INDEX</p><h2>수집 지역</h2></div><span>{filteredRegions.length}건</span></div><label className="search-field"><span className="sr-only">지역 또는 품목 검색</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="지역 또는 품목 검색" /></label><div className="region-list">{filteredRegions.map((region) => { const count = regionTrademarkValue(region); const filing = regionFilingSummary(region); return <button type="button" key={regionKey(region)} className={regionKey(selectedRegion) === regionKey(region) ? "region-button active" : "region-button"} onClick={() => chooseRegion(region)}><span><strong>{region.region}</strong><small>{region.items.length}개 품목 · {count === null ? "데이터 검토 중" : `상표 ${number(count)}건`}{filing ? ` · 출원율 ${percent(filing.rate)} (${filing.filed}/${filing.total})` : ""}</small></span><span className={`state state-${region.dataState}`}>{STATE_LABELS[region.dataState] || region.dataState}</span></button>; })}{filteredRegions.length === 0 && <p className="empty">검색 결과가 없습니다.</p>}</div></aside><RegionDetail region={selectedRegion} item={selectedItem} onItem={setSelectedItemId} /></section></section>}

    {tab === "items" && <section className="screen-section"><div className="screen-heading"><div><p className="eyebrow">ITEM EXPLORER</p><h1>품목별 조회</h1></div><p>품목을 기준으로 지역과 상표 활동을 다시 묶었습니다.</p></div><div className="item-screen"><div className="item-screen-toolbar"><label><span className="sr-only">품목 검색</span><input value={itemQuery} onChange={(event) => setItemQuery(event.target.value)} placeholder="품목 또는 지역 검색" /></label><span>{itemRows.length}개 품목</span></div><p className="ranking-note">표의 숫자는 특산품 개수가 아니라, 이 품목명으로 아래 지역들에 출원된 상표 건수입니다. 등록률 = 등록 완료 건수 ÷ 상표 출원 건수. &quot;주소 확인 중&quot;은 출원인 주소가 이 지역인지 아직 검증되지 않아 건수를 낼 수 없다는 뜻이며, 품목이 없다는 의미가 아닙니다.</p><div className="item-table" role="table"><div className="item-table-head" role="row"><span>품목</span><span>수집 지역</span><span title="출원인 주소가 이 지역으로 확인된 고유 상표 출원 건수 합계">상표 출원 건수</span><span title="위 출원 건수 중 상표가 등록 상태로 확인된 건수">등록 완료 건수</span><span title="등록 완료 건수 ÷ 상표 출원 건수">등록률</span></div>{itemRows.map((row, index) => <div className="item-table-row" role="row" key={row.name}><span><b>{String(index + 1).padStart(2, "0")}</b><strong>{row.name}</strong></span><span>{row.regions.join(", ")}</span><span title={row.verified ? undefined : "상표 출원은 확인됐지만 출원인 주소가 이 지역인지 아직 검증되지 않았습니다."}>{row.verified ? `${number(row.trademarks)}건` : "주소 확인 중"}</span><span title={row.verified ? undefined : "상표 출원은 확인됐지만 출원인 주소가 이 지역인지 아직 검증되지 않았습니다."}>{row.verified ? `${number(row.registered)}건` : "주소 확인 중"}</span><span>{row.verified ? percent(row.trademarks ? row.registered / row.trademarks : null) : "—"}</span></div>)}</div><div className="method-note"><strong>집계 기준</strong><p>고시명칭·NICE류가 확정된 품목만 공식 명칭 기준으로 재그룹합니다(&quot;풋고추&quot;·&quot;파프리카&quot;는 같은 &quot;고추&quot;로 합쳐짐). 아직 고시명칭이 확정되지 않은 검토대기 원물명은 지역별 상세 화면에서 원문 그대로 확인할 수 있습니다.</p></div></div></section>}

    {tab === "compare" && <section className="screen-section"><div className="screen-heading"><div><p className="eyebrow">SPECIALIZED CROP MATCH</p><h1>특화작목 비교</h1></div><p>정책 지정 작목과 실제 상표 활동의 일치 여부를 비교하는 화면입니다.</p></div><div className="compare-banner"><span>현재 상태</span><strong>정책 지정 특화작목 원본 미수집 · 비교 대기</strong><p>농사로 지역특산물과 정책 지정 특화작목은 같은 데이터가 아니므로 임의로 대체하지 않습니다.</p></div><div className="compare-grid">{[...provinceStats.entries()].map(([province, stat]) => { const names = snapshot.regions.filter((region) => (region.sido || region.region) === province).flatMap((region) => region.items.map(itemName)); return <article key={province}><div className="compare-head"><h2>{province}</h2><span>비교 대기</span></div><dl><dt>현재 공식 특산품 후보</dt><dd>{names.join(", ") || "없음"}</dd><dt>정책 지정 특화작목</dt><dd className="missing">미수집</dd><dt>지역 상표 활동</dt><dd>{stat.verified ? `${number(stat.trademarks)}건` : "검증 중"}</dd><dt>일치 여부</dt><dd className="missing">판정 불가</dd></dl></article>; })}</div><div className="compare-sources"><article><span>추가 수집 1</span><strong>농촌진흥청 지역특화작목 지정 목록</strong><p>계획 기간·지역·작목·고시 또는 보고서 버전을 구조화해야 합니다.</p></article><article><span>추가 수집 2</span><strong>한국지식재산연구원 로컬브랜드 근거</strong><p>레퍼런스가 인용한 2024년 보고서의 지역·품목 대응표와 페이지 근거가 필요합니다.</p></article><article><span>자동화 원칙</span><strong>정책 목록 확보 후 결정론적 비교</strong><p>명칭 정규화 후보만 사람 검토하고 나머지 집계·일치 판정은 자동화합니다.</p></article></div></section>}

    {tab === "data" && pipeline && <section className="screen-section data-overview"><div className="screen-heading"><div><p className="eyebrow">DATA JOURNEY</p><h1>특산물과 상표가<br />데이터가 되기까지</h1></div><p>수집한 특산물을 표준화하고 상표·출원인 주소와 연결해 지역별 지표로 만드는 전 과정을 보여줍니다.</p></div><div className="data-flow" aria-label="데이터 처리 흐름"><article><span>01 · 수집 입력</span><strong>{number(pipeline.rowCounts.total)}</strong><small>지역×특산물 원본 행</small></article><i>→</i><article><span>02 · 표준화 완료</span><strong>{number(snapshot.coverage.regionItemCount)}</strong><small>정제된 지역×품목</small></article><i>→</i><article><span>03 · 고유 검색어</span><strong>{number(pipeline.uniqueQueryCounts.total)}</strong><small>고시명칭 + NICE류</small></article><i>→</i><article><span>04 · 상표 매칭</span><strong>{number(pipeline.nationwideCandidates.uniqueTrademarkCount)}</strong><small>출원번호 기준 고유 후보</small></article><i>→</i><article className="flow-highlight"><span>05 · 지역 지표</span><strong>{number(pipeline.regionalMetricGate.availableRegionItemCount)}</strong><small>검증 기준 통과 항목</small></article></div><div className="data-summary-grid"><article className="data-summary-card"><p className="eyebrow">SPECIALTY DATA</p><h2>특산물 데이터</h2><div className="data-stat"><strong>{number(uniqueSpecialtyCount)}개</strong><span>고유 특산품명</span></div><div className="data-stat"><strong>{number(snapshot.coverage.regionItemCount)}개</strong><span>지역×품목 조합</span></div><div className="data-stat"><strong>{number(snapshot.coverage.observedRegionCount)}개</strong><span>관측 지역</span></div><p className="data-card-note">같은 특산물도 지역이 다르면 별도 관측 단위로 관리합니다.</p></article><article className="data-summary-card"><p className="eyebrow">TRADEMARK MATCH</p><h2>상표 매칭 결과</h2><div className="match-bars"><div><span>주소 확보 완료 <b>{number(pipeline.applicantRegionVerification.verifiedCount)}</b></span><em><i style={{ width: `${Math.round((pipeline.applicantRegionVerification.rate || 0) * 100)}%` }} /></em><small>전국 상표 후보 중 출원인 주소를 확인한 비율 {percent(pipeline.applicantRegionVerification.rate)}</small></div><div><span>지역 지표 표시 가능 <b>{number(pipeline.regionalMetricGate.availableRegionItemCount)}</b></span><em><i style={{ width: `${Math.round(pipeline.regionalMetricGate.availableRegionItemCount / Math.max(1, gateTotal) * 100)}%` }} /></em><small>전체 지역×품목 조합 중 지표를 표시할 수 있는 비율 {percent(pipeline.regionalMetricGate.availableRegionItemCount / Math.max(1, gateTotal))}</small></div></div><p className="data-card-note">상표 검색은 품목명 키워드로 전국 후보를 찾는 방식이라, 검색된 상표 중에는 이 지역과 무관한 다른 지역·전국 출원인(주소지 기준)도 많이 섞여 있습니다. 지역 지표(상표 건수·등록률)에는 출원인 주소지가 이 지역으로 확인된 건만 사용합니다.</p></article></div><div className="data-reading-note"><strong>숫자를 읽는 법</strong><p><b>{number(pipeline.rowCounts.total)}행</b>은 수집 입력량, <b>{number(uniqueSpecialtyCount)}개</b>는 고유 특산품명, <b>{number(pipeline.uniqueQueryCounts.total)}개</b>는 실제 API 검색 조합입니다. <b>{number(pipeline.nationwideCandidates.uniqueTrademarkCount)}건</b>은 전국 상표 후보이며, 지역 지표에는 출원인 주소와 수집률 기준을 통과한 값만 사용합니다.</p></div></section>}
