...
// 이슈 #118: 최근 출원·등록 동향 리더보드. 요약 탭 하단에 compact로 넣는다(별도 "동향"
// 탭은 #119 IA 축소 방향과 상충해 사용자가 요약 흡수를 선택, 2026-09-08). 월 단위 집계
// (applicationMonthCounts/registrationMonthCounts, 키 YYYY-MM)는 ④ analyzer가 전국 키워드
// 검색 결과 전체를 대상으로 채운다 — 출원인 주소로 지역 귀속이 확인된 uniqueTrademarkCount와는
// 다른 모집단이라, 리더보드는 "전국 규모 최근 동향"으로 라벨링한다(#50).
const LEADER_WINDOWS: { months: number; label: string }[] = [
...
function ymKey(year: number, monthIndex0: number) { return `${year}-${String(monthIndex0 + 1).padStart(2, "0")}`; }
// KIPRIS 출원공개 지연 + 주간 갱신이라 생성월(진행 중)은 과소 집계된다 — 직전 월까지를
// "최근"의 끝으로 삼는다.
function leaderEndMonth(generatedAt: string): { year: number; monthIndex0: number } {
  const date = new Date(generatedAt);
  const prev = new Date(date.getFullYear(), date.getMonth() - 1, 1);
  return { year: prev.getFullYear(), monthIndex0: prev.getMonth() };
}
function monthWindowKeys(end: { year: number; monthIndex0: number }, months: number): string[] {
  const keys: string[] = [];
  for (let offset = months - 1; offset >= 0; offset--) {
    const date = new Date(end.year, end.monthIndex0 - offset, 1);
    keys.push(ymKey(date.getFullYear(), date.getMonth()));
  }
  return keys;
}
function sumMonthCounts(counts: Record<string, number> | null | undefined, keys: string[]): number {
  if (!counts) return 0;
  let total = 0;
  for (const key of keys) total += counts[key] || 0;
  return total;
}
function monthRangeLabel(keys: string[]): string {
  if (keys.length === 0) return "";
  const fmt = (key: string) => { const [y, m] = key.split("-"); return `${y}.${m}`; };
  return keys.length === 1 ? fmt(keys[0]) : `${fmt(keys[0])}–${fmt(keys[keys.length - 1])}`;
}
...
// 이슈 #136(2026-09-07): 상표 데이터가 1954년까지 거슬러 올라가지만 2000년 이전은
// 표본이 희박해 그래프 왼쪽이 대부분 바닥에 붙어 최근 추세를 읽기 어렵다는 지적이
// 반복됐다. 추이 그래프의 표시 범위(슬라이더 하한 포함)를 2000년으로 고정하고,
// 기본 구간도 2000년~최근 전체를 연다(협업자 2026-09-07 "그냥 2000년 이후만").
const TREND_FLOOR_YEAR = 2000;
function flooredTrendStart(sortedYears: number[]): number {
  if (sortedYears.length === 0) return TREND_FLOOR_YEAR;
...
function defaultTrendStart(fullStart: number) {
  return fullStart;
}
...
function RegionTrend({ region, heading = "연도별 출원·등록 추이", subtitle, prominent = false, adjustable = false, emptyLabel = "이 지역은 아직 연도별 데이터가 없습니다." }: { region: Pick<Region, "region" | "items">; heading?: string; subtitle?: string; prominent?: boolean; adjustable?: boolean; emptyLabel?: string }) {
  ...
  const fullStart = flooredTrendStart(allYears);
...
// 이슈 #136(2026-09-07): 비즈니스 전략 표에 쓰는 전국 검색 건수와 "전국 대비" 비중.
// 분모(전국 검색)는 지역 귀속 검증 전 후보 모집단이라 확정 수치와 성격이 다르다 —
// 두 값을 더하지 않고, 화면에도 모집단이 다르다는 각주를 함께 둔다.
// 검색 상한(cap 1,800)에 걸린 품목은 전국 검색 값이 잘린 수치라 "1,800건+"로 적고,
// 그 값을 분모로 쓴 비중은 실제보다 크게 나오므로 "n% 이하"로 적는다(2026-09-07 스냅샷
// 기준 1,932개 지역×품목 중 411개가 상한 도달).
function nationwideReach(item: Item): { count: number | null; share: number | null; capped: boolean } {
  ...
  const share = local.availability === "available" && typeof local.value === "number" && count !== null && count > 0
    ? local.value / count
    : null;
  return { count, share, capped };
}
function nationwideCountLabel(count: number | null, capped: boolean) { return count === null ? "—" : `${number(count)}건${capped ? "+" : ""}`; }
function nationwideShareLabel(share: number | null, capped: boolean) { return share === null ? "—" : capped ? `${percent(share)} 이하` : percent(share); }
...
// 이슈 #116(2026-08-26): 출원번호 앞 2자리로 단체표장(44)·증명표장(48)을 구분해달라는 요청.
// 2026-09-08 정정: 44를 "GI 단체표장"으로 단정하고 있었으나, 컨설팅 보고서(Ⅲ장 1.2.4)의
// 정리대로 44는 단체표장 전체이고 지리적 표시 단체표장을 "포함"할 뿐이다. 48도 증명표장
// 전체다. 지리적표시 여부는 개별 확인 대상이므로 표장 종류만 적는다.
const MARK_TYPE_LABELS: Record<string, string> = { "40": "일반상표", "44": "단체표장", "48": "증명표장" };
...
// 판정어는 04-analyze-brand/lib/nationwideFlow.js의 목록과 같은 기준을 쓰되,
// 그쪽은 지정상품이 없어 상표명에 적용하는 근사치이고 이쪽은 지정상품 원문에 적용한다.
...
function goodsStageOf(designatedProductName?: string | null): "raw" | "processed" | null {
...
function goodsStageLabel(evidence: { designatedProductName?: string | null; classCode?: string | null }[]) {
...
// 컨설팅 보고서 Ⅲ장 4.2 「권리 확보보다 권리 점검이 시급하다」의 진단을 화면 지표로 옮긴다.
// 확인한 4건 중 3건이 "행위(35류 이상 서비스업)는 보호되는데 물건(1~34류 상품)은 보호되지
// 않는" 상태였고, 자료상으로는 모두 "상표 보유"로 기재돼 문제가 드러나지 않았다.
// 출원 건수만으로는 이 구분이 안 되므로 지정상품의 류로 갈라 네 칸으로 표시한다.
// 지정상품은 전체의 5.5%만 확보돼 있어(단어검색 응답에 지정상품이 없음) 나머지는
// "권리 내용 미확인"으로 남긴다 — 미확인 자체가 실사 대상이라는 뜻이다.
type RightsStatus = { key: "none" | "service-only" | "goods" | "unknown" | "pending"; label: string; note: string };
function rightsStatusOf(item: Item): RightsStatus {
  const metric = item.metrics.uniqueTrademarkCount;
  if (metric.availability !== "available") {
    return { key: "pending", label: "집계 대기", note: "지역 귀속 확인이 끝나면 판정합니다" };
  }
  if (!(metric.value || 0)) {
    return { key: "none", label: "무권리", note: "지역 주소가 확인된 출원이 없습니다 — 권리화 1순위" };
  }
  const classes = (item.trademarkExamples || [])
    .flatMap((example) => (example.goodsEvidence || []).map((row) => Number(row.classCode)))
    .filter((code) => Number.isFinite(code) && code > 0);
  if (!classes.length) {
    return { key: "unknown", label: "권리 내용 미확인", note: "출원은 있으나 지정상품이 확인되지 않았습니다 — KIPRIS로 실사 필요" };
  }
  const hasGoods = classes.some((code) => code <= 34);
  const hasService = classes.some((code) => code >= 35);
  if (!hasGoods && hasService) {
    return { key: "service-only", label: "행위만 보호", note: "서비스업(35류 이상)만 확인되고 상품 류가 없습니다 — 제품에 붙일 권리가 없습니다" };
  }
  return { key: "goods", label: hasService ? "상품류·서비스류 보유" : "상품류 보유", note: "확인된 지정상품에 상품 류(1~34류)가 있습니다" };
}
function markTypeOf(applicationNumber?: string | null) { return applicationNumber ? MARK_TYPE_LABELS[applicationNumber.slice(0, 2)] || null : null; }
// 등록 사례 카드의 칩은 일반상표(40)까지 달면 거의 모든 카드에 붙어 정보가 되지 않는다 —
// 눈에 띄어야 하는 단체표장·증명표장만 칩으로 남긴다.
function giMarkLabel(applicationNumber?: string | null) { const label = markTypeOf(applicationNumber); return label && label !== "일반상표" ? label : null; }
// 지역×품목의 표장 종류 분포. trademarkExamples는 최근 10건 표본(maxRecentBrands)이므로
// 전체 건수가 아니라 "표본에서 확인된 수"로만 표기해야 한다.
...
function GiHoldingsRow({ item, examples }: { item: Item; examples: TrademarkExample[] }) {
  const registry = (item.sources || []).some((source) => GI_REGISTRY_SOURCES.has(source));
  ...
  const cell = (label: string, held: boolean, detail: string, hint: string) =>
    <em className={held ? "gi-holding held" : "gi-holding none"} title={hint}><b>{label}</b><span>{detail}</span></em>;
  return <div className="gi-holdings-row">
    <strong>지리적표시</strong>
    <span className="gi-holdings">
...
function markTypeBreakdown(examples: { applicationNumber?: string | null }[]) {
  const counts = new Map<string, number>();
  for (const example of examples) {
    const label = markTypeOf(example.applicationNumber) || "기타";
...
// 쓰는 기준은 후자이므로 류 기준 분포를 따로 보여준다.
// 2026-09-08 정정(사용자): 29·30·31류는 지정상품이 무엇이냐에 따라 원물일 수도 가공품일
// 수도 있어 류만으로 가를 수 없다. 류 단위로 확실히 갈리는 것은 제품(1~34류)과
// 서비스·확산(35류 이상)뿐이고, 원물/가공품 구분은 지정상품명을 봐야 한다.
const REPORT_CHAIN: { key: string; label: string; test: (code: number) => boolean; hint: string }[] = [
  { key: "goods", label: "제품", test: (code) => code >= 1 && code <= 34, hint: "1~34류" },
  { key: "service", label: "서비스·확산", test: (code) => code >= 35, hint: "35류 이상" },
];
// 각 단계의 classes는 상위 5개만 실려 있으므로(analyzeNationwideFlow) 합계는 하한이다.
// 한 상표가 여러 류를 가지면 류마다 세므로 상표 건수와도 다르다 — 문구로 못박는다.
function reportChainBuckets(flow: NationwideFlow) {
  const byClass = new Map<string, number>();
  for (const key of ["raw", "processed", "service"] as const) {
    for (const row of flow.stages[key].classes || []) {
      byClass.set(row.classCode, (byClass.get(row.classCode) || 0) + row.count);
    }
  }
  const buckets = REPORT_CHAIN.map((bucket) => {
    ...
    return {
      ...
      hitClasses: entries.sort((a, b) => b[1] - a[1]).map(([code]) => code),
    };
  });
...
type ClassCell = { count: number; goods: Set<string> };
type ExpansionEntry = { name: string; category: ItemCategory | null; classes: Map<string, ClassCell> };
type ExpansionIndex = {
  byItem: Map<string, ExpansionEntry>;
  byClass: Map<string, { items: Map<string, Set<string>>; count: number }>;
  byCategory: Map<string, Map<string, Set<string>>>;
  byTemplate: Map<string, Map<string, Set<string>>>;
};
// 「신선한 토마토」의 지정상품 문안은 「토마토」를 담고 있다 — 수식 접두어를 떼야 맞물린다.
const TEMPLATE_SLOT = "{품목}";
const bareItemName = (name: string) => String(name || "").replace(/^(신선한|미가공|말린|건조된|생)\s*/, "").replace(/\s+/g, "");
const hasFinalConsonant = (word: string) => {
  const code = String(word || "").trim().slice(-1).charCodeAt(0);
  if (!(code >= 0xac00 && code <= 0xd7a3)) return null;
  return (code - 0xac00) % 28 !== 0;
};
const withTopicJosa = (word: string) => `${word}${hasFinalConsonant(word) === false ? "는" : "은"}`;
const normalizeGoods = (text: string) => String(text || "").replace(/[\s()（）]/g, "");
const isTautology = (goods: string, itemName: string) => normalizeGoods(goods) === normalizeGoods(itemName);
// 읽는 사람이 알고 싶은 건 "이 품목이 무엇으로 사업이 됐는가"이므로, 서비스업(…업)과
// 가공품을 원물 나열보다 먼저 보여 준다. 품목명을 그대로 옮겨 적은 행은 가장 뒤로.
const BUSINESS_MARKERS = /(업|가공|제조|체험|판매|배달|음료|주스|막걸리|맥주|와인|차|빵|과자|잼|즙|진액|분말|가루|말랭이|절임|김치|장아찌|통조림|엑기스|화장품|비누)/;
function showcaseScore(goods: string, itemName: string) {
  if (!goods) return -1;
...
function classShowcase(cell: { items: Map<string, Set<string>> }) {
  let best: { lead: string; goods: string; score: number } | null = null;
...
function goodsSample(set: Set<string>, n: number, itemName?: string) {
  const all = [...set];
...
function buildExpansionIndex(regions: Region[]): ExpansionIndex {
  const byItem = new Map<string, ExpansionEntry>();
  const byClass = new Map<string, { items: Map<string, Set<string>>; count: number }>();
  ...
  for (const region of regions) {
    for (const item of region.items) {
      ...
      if (!name) continue;
      const entry = byItem.get(name) || { name, category: item.category || null, classes: new Map<string, ClassCell>() };
      ...
      for (const example of item.trademarkExamples || []) {
        for (const evidence of example.goodsEvidence || []) {
          ...
          const goods = String((evidence as { designatedProductName?: string }).designatedProductName || "").trim();
          if (!code || !Number.isFinite(Number(code))) continue;
          const own = entry.classes.get(code) || { count: 0, goods: new Set<string>() };
          own.count += 1;
          ...
          // 지어낸 문구가 아니라 실제 등록된 표현을 그대로 옮겨 쓰는 것이다. 괄호가 붙은 틀은
          // 특정 지역·원료를 한정하는 조건절이라 남의 품목에 옮겨 붙일 수 없어 뺀다.
          ...
          if (bare && goods.includes(bare)) {
            ...
            if (shape !== TEMPLATE_SLOT && !/[()（）[\]]/.test(shape)) {
              const perClass = byTemplate.get(code) || new Map<string, Set<string>>();
              const users = perClass.get(shape) || new Set<string>();
              users.add(bare);
              perClass.set(shape, users);
              byTemplate.set(code, perClass);
            }
          }
          const shared = byClass.get(code) || { items: new Map<string, Set<string>>(), count: 0 };
          shared.count += 1;
          const bucket = shared.items.get(name) || new Set<string>();
  ...
  for (const entry of byItem.values()) {
    ...
    if (!code) continue;
    const map = byCategory.get(code) || new Map<string, Set<string>>();
    for (const cls of entry.classes.keys()) {
      const names = map.get(cls) || new Set<string>();
...
// 두 품목 이상이 실제로 쓴 틀만 제안한다 — 놓치는 제안이 생기더라도 말이 안 되는 제안을
// 내놓는 것보다 낫다. 제안이 하나도 없으면 그 줄은 근거만 보여 준다.
const SUGGEST_MIN_USERS = 2;
function suggestGoods(index: ExpansionIndex, code: string, itemName: string, limit: number) {
...
const SuggestionLine = ({ list }: { list: string[] }) => list.length
  ? <small className="expansion-suggest"><b>제안</b> {list.map((text) => `「${text}」`).join(" · ")}</small>
  : null;
...
// 않도록 별도 섹션에 넣고, 모든 수치·문안에 예시 표시를 단다. standalone-client.js의
// SAMPLE_EXPANSION_REPORTS와 같은 내용.
type SampleRow = { code: string; why: string; suggest: string[]; note?: string };
type SampleReport = {
  name: string; region: string; category: string; lede: string;
  held: { code: string; count: number; goods: string[] }[];
  next: SampleRow[]; unusual: SampleRow[]; service: SampleRow[]; regional: string;
};
const SAMPLE_EXPANSION_REPORTS: SampleReport[] = [
  {
    "name": "딸기",
    "region": "경상남도 진주시",
    "category": "과일",
...
function SampleSection({ title, hint, rows, kind }: { title: string; hint: string; rows: SampleRow[]; kind: string }) {
  return <article>
    <h3>{title}</h3>
    <p className="expansion-hint">{hint}</p>
    <ol className={kind ? `expansion-list ${kind}` : "expansion-list"}>{rows.map((row) => <li key={row.code}>
      <span className="expansion-class">{niceClassLabel(row.code)}</span>
      <span className="expansion-why">{row.why}</span>
      <small className="expansion-suggest"><b>제안</b> {row.suggest.map((text) => `「${text}」`).join(" · ")}</small>
      {row.note && <small className="expansion-basis">{row.note}</small>}
    </li>)}</ol>
  </article>;
}
function SampleReportCard({ sample }: { sample: SampleReport }) {
  return <section className="expansion-report sample">
    <div className="section-heading"><div><h2>{sample.name} 확장 경로 진단<em className="sample-tag">예시</em></h2></div><span>{sample.category} · {sample.region}</span></div>
    <p className="expansion-lede" dangerouslySetInnerHTML={{ __html: sample.lede }} />
    <div className="expansion-grid">
      <article>
        <h3>① 지정상품이 확인된 범위</h3>
        <ul className="expansion-held">{sample.held.map((row) => <li key={row.code} className={Number(row.code) >= REPORT_SERVICE_FLOOR ? "svc" : "goods"}>
          <b>{niceClassLabel(row.code)}</b><span>{number(row.count)}건</span><small>{row.goods.join(" · ")}</small>
        </li>)}</ul>
      </article>
      <SampleSection title="② 다음 확장 후보" hint="같은 유형이 이미 확보했는데 이 품목만 비어 있는 상품류입니다." rows={sample.next} kind="" />
      <SampleSection title="③ 이색 확장 사례" hint="선례가 드문 만큼 차별화 여지도 큰 구간입니다." rows={sample.unusual} kind="rare" />
      <SampleSection title="④ 서비스 · 지역 창업" hint="제품이 아니라 파는 행위·체험에 붙이는 권리입니다(35류 이상)." rows={sample.service} kind="svc" />
      <article className="expansion-region">
        <h3>⑤ {sample.region} 관점</h3>
        <p className="expansion-hint" dangerouslySetInnerHTML={{ __html: sample.regional }} />
      </article>
    </div>
  </section>;
}
function SampleReports() {
  return <details className="sample-reports">
    <summary><span>예시 보고서 {number(SAMPLE_EXPANSION_REPORTS.length)}건 보기</span><small>{SAMPLE_EXPANSION_REPORTS.map((s) => s.name).join(" · ")}</small></summary>
    <p className="sample-banner"><b>이 다섯 건은 데이터가 아닙니다.</b> 지정상품 대조가 질의한 상품류 안쪽만 남기는 구조라, 지금 집계로는 확장 경로가 원물 한 칸으로 눌립니다. 데이터가 갖춰졌을 때 이 화면이 무엇을 보여 주는지를 <b>사람이 손으로 쓴 가상 예시</b>입니다 — 건수·문안 모두 실제 출원 현황이 아니며 그대로 인용하면 안 됩니다.</p>
  ...
  return <article className="expansion-region">
    ...
    {localSvc.length > 0
      ? <ol className="expansion-list svc">{localSvc.map((row) => <li key={row.code}>
        ...
        <small className="expansion-basis">근거 {row.show!.lead} 「{row.show!.goods}」</small>
      </li>)}</ol>
...
function ExpansionReportCard({ index, name, category, province, regions }: { index: ExpansionIndex; name: string; category: ItemCategory | null; province: string; regions: Region[] }) {
  ...
  if (!evidenceCount) {
    return <section className="expansion-report empty">
      <div className="section-heading"><div><h2>{name} 확장 경로 진단</h2></div><span>근거 부족</span></div>
      <p className="empty">이 품목은 출원의 지정상품이 아직 한 건도 확인되지 않아 확장 경로를 진단할 수 없습니다. 지정상품 대조가 끝나면 자동으로 채워집니다.</p>
    </section>;
  }
  const ownCodes = [...own.keys()].sort((a, b) => Number(a) - Number(b));
  const productCodes = ownCodes.filter((code) => Number(code) < REPORT_SERVICE_FLOOR);
  const serviceCodes = ownCodes.filter((code) => Number(code) >= REPORT_SERVICE_FLOOR);

  const catCode = (entry?.category || category)?.code || "";
  const peerMap = index.byCategory.get(catCode) || new Map<string, Set<string>>();
  // 같은 유형 안에서 단 한 품목만 가진 류는 대개 분류 잡음이다(쌀 상표 한 건이 2류로 잡힌 것
  // 같은). 두 품목 이상이 확보한 류만 후보로 올리고, 세 줄이 안 되면 전국 기준으로 채운다.
  const peerRows: { code: string; scope: string; peers: number; lead: string; goods: string }[] = [...peerMap.entries()]
    .filter(([code, names]) => !own.has(code) && [...names].filter((n) => n !== name).length >= 2)
    .map(([code, names]) => {
      const peers = [...names].filter((n) => n !== name);
      const show = classShowcase(index.byClass.get(code)!) || { lead: peers[0], goods: "" };
      return { code, scope: (entry?.category || category)?.label || "같은 유형", peers: peers.length, lead: show.lead, goods: show.goods };
    })
    .sort((a, b) => b.peers - a.peers || Number(a.code) - Number(b.code));
  if (peerRows.length < 3) {
    const seen = new Set(peerRows.map((row) => row.code));
    peerRows.push(...[...index.byClass.entries()]
  ...
  // 「담배」가 34류에 「담배」로 등록된 것 같은 동어반복은 이색 사례가 못 된다.
  const rare = [...index.byClass.entries()]
    ...
    .filter((row) => row.show && row.show.goods && !isTautology(row.show.goods, row.show.lead))
    ...
    <div className="expansion-grid">
      <article>
        ...
        <ul className="expansion-held">{ownCodes.map((code) => {
          ...
          return <li key={code} className={Number(code) >= REPORT_SERVICE_FLOOR ? "svc" : "goods"}>
            <b>{niceClassLabel(code)}</b><span>{number(cell.count)}건</span>
            {eg.length > 0 && <small>{eg.join(" · ")}</small>}
          </li>;
        })}</ul>
      </article>
      <article>
        <h3>② 다음 확장 후보</h3>
        <p className="expansion-hint">같은 유형의 다른 특산품이 이미 확보했는데 이 품목만 비어 있는 상품류입니다. 세 줄이 안 되면 전국 특산품 기준으로 채웁니다.</p>
        {peerRows.length > 0
          ? <ol className="expansion-list">{peerRows.map((row) => <li key={row.code}>
            <span className="expansion-class">{niceClassLabel(row.code)}</span>
            <span className="expansion-why">{row.scope} {number(row.peers)}개 품목이 이미 확보</span>
            <SuggestionLine list={suggestGoods(index, row.code, name, 3)} />
            {row.goods && <small className="expansion-basis">근거 {row.lead} 「{row.goods}」</small>}
          </li>)}</ol>
          : <p className="empty">같은 유형 품목들이 확보한 상품류를 이 품목도 모두 갖고 있습니다.</p>}
      </article>
      <article>
        <h3>③ 이색 확장 사례</h3>
        <p className="expansion-hint">전국 특산품 상표에서 도달한 품목이 가장 적은 상품류입니다 — 선례가 드문 만큼 차별화 여지도 큽니다.</p>
        {rare.length > 0
          ? <ol className="expansion-list rare">{rare.map((row) => <li key={row.code}>
            <span className="expansion-class">{niceClassLabel(row.code)}</span>
            <span className="expansion-why">전국 특산품 중 {number(row.itemCount)}개 품목만 도달</span>
            <SuggestionLine list={suggestGoods(index, row.code, name, 2)} />
            <small className="expansion-basis">근거 {row.show!.lead} 「{row.show!.goods}」</small>
          </li>)}</ol>
          : <p className="empty">이 품목이 이미 대부분의 상품류에 도달해 있습니다.</p>}
      </article>
      <article>
        <h3>④ 서비스 · 지역 창업</h3>
        ...
        {svcRows.length > 0
          ? <ol className="expansion-list svc">{svcRows.map((row) => <li key={row.code}>
            <span className="expansion-class">{niceClassLabel(row.code)}</span>
            <span className="expansion-why">전국 특산품 상표 {number(row.count)}건이 이 구간에</span>
            <SuggestionLine list={suggestGoods(index, row.code, name, 3)} />
            {row.show && row.show.goods && <small className="expansion-basis">근거 {row.show.lead} 「{row.show.goods}」</small>}
          </li>)}</ol>
...
function NationwideFlowCard({ flow, itemLabel, origins }: { flow: NationwideFlow; itemLabel: string; origins?: string[] }) {
  ...
  return (
    <section className="nationwide-flow-card">
      ...
      {chain.leader && <div className="report-chain">
        <div className="report-chain-head">
          <strong>보고서 밸류체인 기준 상품류 활동</strong>
          <span>기축 <b>{chain.leader.label}</b> · {chain.leader.hitClasses.slice(0, 4).join("·")}류</span>
        </div>
        <div className="report-chain-rows">
          {chain.buckets.map((bucket) => <Fragment key={bucket.key}>
            <span className="report-chain-label">{bucket.label}<small>{bucket.hint}</small></span>
            <span className="report-chain-track"><i style={{ width: `${Math.round(bucket.count / chain.max * 100)}%` }} /></span>
            <span className="report-chain-value">{bucket.count ? number(bucket.count) : "—"}</span>
          </Fragment>)}
        </div>
...
// 이슈 #136(협업자 2026-09-06): "DB 내에서 찾은 발견 외에, 비즈니스 확장 방향을 제안해
...
function expansionSuggestions(flow: NationwideFlow, opts: { surging?: boolean } = {}): { kind: string; text: string }[] {
  ...
  const rawRegion = raw.topRegions?.[0]?.region;
  const processedRegion = processed.topRegions?.[0]?.region;
  const topRawClass = raw.classes?.[0];
  ...
  if (opts.surging) {
    out.push({ kind: "momentum", text: "최근 출원이 급증하는 품목입니다 — 선점 경쟁이 빨라지고 있어 조기 출원 전략이 필요합니다." });
  }
  if (out.length === 0 && service.count > processed.count * 0.3) {
    out.push({ kind: "balanced", text: "원물·가공·서비스 전 단계에 상표 활동이 고르게 있습니다 — 지역 특화 세부 상품류를 겨냥한 차별화가 다음 과제입니다." });
  }
  return out.slice(0, 4);
}
function ExpansionSuggestionsCard({ flow, itemLabel, surging }: { flow: NationwideFlow; itemLabel: string; surging?: boolean }) {
  const suggestions = expansionSuggestions(flow, { surging });
  if (suggestions.length === 0) return null;
  return (
    <section className="expansion-card">
      <div className="section-heading"><div><h2>{itemLabel} 확장 방향 제안</h2></div><span>전국 흐름 데이터 기반</span></div>
      <ul className="expansion-list">{suggestions.map((s) => <li key={s.kind}><span className="expansion-dot" aria-hidden="true" />{s.text}</li>)}</ul>
    </section>
  );
}
  ...
  return (
    <section className={briefing.isGapAlert ? "business-strategy alert" : "business-strategy"}>
      ...
      {hasStats && <div className="strategy-stat-row">
        ...
        {/* 이슈 #136(2026-09-07): 지역 건수만으로는 규모를 가늠할 수 없어 전국 검색 건수와
            그 대비 비중을 함께 둔다. 전국 검색은 지역 확인 전 후보라 모집단이 다르다. */}
        {nationwideCount !== null && <div className="strategy-stat"><span title={`같은 품목명의 전국 검색 결과 · 지역 확인 전 후보라 위 확정 건수와는 다른 모집단${nationwideCapped ? " · 검색 상한(1,800건)에 걸려 실제로는 더 많습니다" : ""}`}>전국 검색</span><strong>{nationwideCountLabel(nationwideCount, nationwideCapped)}</strong></div>}
        {nationwideShare !== null && <div className="strategy-stat"><span title="지역 확인 출원 ÷ 전국 검색 결과">전국 대비</span><strong>{nationwideShareLabel(nationwideShare, nationwideCapped)}</strong></div>}
      ...
      {/* 이슈 #136 코멘트(2026-09-03) 09번: briefing.sentences는 파이프라인이 생성 시점에
          원본 시도명을 그대로 문장에 새겨 넣어(예: "전남광주통합특별시 영광군은...") 화면
...
const itemGapRegions = (row: { availableRegions: string[]; regionCounts: Record<string, number> }) =>
  row.availableRegions.filter((region) => !(row.regionCounts[region] > 0)).length;
// 지역의 공백 = 수집된 특산품 중 출원이 확인되지 않은 수.
const coverageGap = (coverage: { total: number; applied: number }) => Math.max(0, coverage.total - coverage.applied);
export default function Dashboard({ snapshot, geometry, registrationExamples }: { snapshot: Snapshot; geometry: MapGeometry; registrationExamples: VerifiedRegistrationExamples }) {
  ...
  const [itemSort, setItemSort] = useState<"gap" | "trademarks" | "regions" | "registrationRate" | "name">("gap");
  const [itemGapOnly, setItemGapOnly] = useState(false);
  ...
  // 정렬 대상이 된다.
  const [strategySortKey, setStrategySortKey] = useState<"verdict" | "region" | "item" | "trademark" | "nationwide" | "nationwideShare" | "registration" | "share">("verdict");
  ...
  const [strategyPolicyOnly, setStrategyPolicyOnly] = useState(false);
  ...
  // 이슈 #118: 동향(리더보드) 화면의 시간 윈도우·기준 토글.
  const [leaderMonths, setLeaderMonths] = useState(3);
  const [leaderMetric, setLeaderMetric] = useState<"application" | "registration">("application");
  const [regionQuery, setRegionQuery] = useState("");
  // 이슈 #136(2026-09-07): 지역별 조회도 품목별 조회와 같은 자리에 같은 모양의 정렬을 둔다.
  const [regionSort, setRegionSort] = useState<"gap" | "name" | "coverage" | "total" | "applied">("gap");
  // 2026-09-08 요청: 품목별 화면에서 지역 칩을 누르면 그 지역의 출원 상표를 바로 본다.
  const [itemRegionPick, setItemRegionPick] = useState("");
  ...
  function applyNavSetters(nav: NavParams) {
    ...
    if (nav.tab !== "applications") { setSelectedProvince(null); setSelectedMunicipality(null); }
  ...
  const itemRows = useMemo(() => {
    ...
    return [...rows.values()]
      ...
      .filter((row) => !itemGapOnly || itemGapRegions(row) > 0)
      .sort((a, b) => b.trademarks - a.trademarks);
  }, [itemQuery, categoryFilter, itemGapOnly, snapshot.regions]);
  ...
  const itemSortedRows = useMemo(() => {
    ...
    if (itemSort === "gap") rows.sort((a, b) => itemGapRegions(b) - itemGapRegions(a) || b.regions.length - a.regions.length || a.name.localeCompare(b.name, "ko-KR"));
    else if (itemSort === "regions") rows.sort((a, b) => b.regions.length - a.regions.length || b.trademarks - a.trademarks);
  ...
  function chooseRegion(region: Region, options?: { scroll?: boolean }) {
    const province = region.sido || region.region;
    ...
    // 지자체 목록에서 고를 때는 목록이 그대로 보여야 하므로 맨 위로 올리지 않는다.
  ...
  function openMunicipality(name: string, options?: { scroll?: boolean }) { setSelectedMunicipality(name); const match = findMunicipalityRegion(selectedProvince, name); if (match) chooseRegion(match, options); }
  ...
  // 이슈 #118: 동향(리더보드) 집계. 품목 순위는 품목별 조회와 같은 기준(officialItemLabel로
  // 고시명칭 확정 품목만 하나로 묶음)을 쓰고, 카테고리 롤업은 category가 붙은 모든 품목행을
  // 대상으로 한다(고시명칭 미확정 원물도 유형은 있으므로).
  const leaderboard = useMemo(() => {
    const end = leaderEndMonth(snapshot.generatedAt);
    const windowKeys = monthWindowKeys(end, leaderMonths);
    const priorEndDate = new Date(end.year, end.monthIndex0 - leaderMonths, 1);
    const priorKeys = monthWindowKeys({ year: priorEndDate.getFullYear(), monthIndex0: priorEndDate.getMonth() }, leaderMonths);
    const minBase = leaderMinBase(leaderMonths);
    type ItemAgg = { name: string; category: ItemCategory | null; app: number; reg: number; priorApp: number; priorReg: number };
    type CatAgg = { label: string; code: string | null; app: number; reg: number; priorApp: number; priorReg: number };
    ...
    for (const region of regionalRegions) {
      for (const item of region.items) {
        ...
        // 카테고리 순위는 유형이 실제로 매겨진 품목행만 — 미분류(전국 검색 노이즈 다수)를
        // "기타"로 뭉치면 순위가 왜곡된다(#118 열린 질문).
        if (item.category) {
    ...
    const metricValue = (row: { app: number; reg: number }) => leaderMetric === "application" ? row.app : row.reg;
    ...
    const lifetimeRows = [...lifetime.values()]
      // 등록 수가 출원 수를 넘는 행은 데이터 아티팩트(등록일만 있고 출원일 파싱 실패 등) —
      // 등록률 순위에서 제외한다.
      .filter((row) => row.lifeApp >= 20 && row.lifeReg <= row.lifeApp)
  ...
  // 지역별 목록 행의 기본 형태 — 광역(도 전체) 행은 region이 없고 시군구 행은 있다.
  // 두 갈래를 같은 타입으로 맞춰야 아래 filter/sort가 유니언 배열로 갈라지지 않는다.
  type CoverageRowBase = { key: string; label: string; regions: Region[]; region: Region | null };
  const coverageBreakdown = ((selectedProvince
    ...
    : [...provinceStats.keys()].map((province) => ({ key: province, label: province, regions: snapshot.regions.filter((region) => region.sido === province), region: null }))) as CoverageRowBase[])
    .map((row) => ({
      ...row,
  ...
  // 자리에서 따로 보여주고, 시군구는 목록에서 골라 상세로 연다(#136, 2026-09-07).
  type CoverageRow = (typeof coverageBreakdown)[number];
  // 이슈 #136(2026-09-07): 지역별 조회의 지자체 목록은 선택과 무관하게 "도 전체" 기준으로
  // 만든다. 이전에는 coverageBreakdown(=선택한 시군구로 이미 좁혀진 목록)으로 그려서,
  // 시군구를 하나 고르는 순간 목록이 그 한 곳으로 줄고 "시군구별 보기" 토글이 사라져
  // 다른 시군구로 옮겨갈 방법이 "← 전국"밖에 없었다.
  const provinceCoverageRows: CoverageRow[] = selectedProvince
    ? snapshot.regions
        .filter((region) => region.sido === selectedProvince)
        .map((region) => ({
          key: regionKey(region),
          label: region.sigungu || region.region,
          regions: [region],
          region,
          coverage: specialtyCoverage([region]),
          items: region.items.map((item) => ({ region, item, label: officialItemLabel(item) || itemName(item) })),
        }))
    : [];
  const sortCoverageRows = (rows: CoverageRow[]) => [...rows].sort((a, b) => {
    if (regionSort === "gap") return coverageGap(b.coverage) - coverageGap(a.coverage) || b.coverage.total - a.coverage.total || a.label.localeCompare(b.label, "ko-KR");
    if (regionSort === "coverage") return (b.coverage.rate ?? -1) - (a.coverage.rate ?? -1);
    if (regionSort === "total") return b.coverage.total - a.coverage.total;
    if (regionSort === "applied") return b.coverage.applied - a.coverage.applied;
  ...
  function coverageCard(row: CoverageRow) {
    const specialtyButtons = row.items.map(({ region, item, label }) => {
      ...
      return <button type="button" key={`${regionKey(region)}-${item.specialtyId}`} onClick={() => { chooseRegion(region); setSelectedItemId(item.specialtyId || ""); setTab("regions"); }}><span>{selectedProvince ? label : `${region.sigungu || displayRegionName(region.region)} / ${label}`}{item.regionalSpecialtyCropBadge && <em className={`crop-badge crop-badge-${item.regionalSpecialtyCropBadge.tier}`}>{item.regionalSpecialtyCropBadge.tier}</em>}</span><small className={`specialty-status ${status.filed ? "filed" : "unfiled"}`}>{status.label}</small></button>;
    });
    ...
    return <article className={selectedMunicipality && row.label === selectedMunicipality ? "coverage-region-card selected" : "coverage-region-card"} key={row.key}><div className="coverage-region-head"><div><strong>{displayRegionName(row.label)}</strong><small>특산품 {number(row.coverage.total)}개</small></div><div className="coverage-region-summary">{coverageGap(row.coverage) > 0 && <span className="coverage-gap-flag" title="수집된 특산품 중 출원이 확인되지 않은 수">공백 {number(coverageGap(row.coverage))}개</span>}<span>출원 확인 특산품 {number(row.coverage.applied)}개</span><b>{percent(row.coverage.rate)}</b></div>{!selectedProvince && <button type="button" onClick={() => openProvince(row.label)}>지도에서 보기</button>}</div>{specialtyList}</article>;
  }
  ...
  function coverageListRow(row: CoverageRow) {
    return <button type="button" className="coverage-region-list-row" key={row.key} onClick={() => openProvince(row.label)}>
      ...
      {coverageGap(row.coverage) > 0 && <span className="coverage-gap-flag" title="수집된 특산품 중 출원이 확인되지 않은 수">공백 {number(coverageGap(row.coverage))}개</span>}<span>출원 확인 {number(row.coverage.applied)}개</span>
    ...
    for (const region of regionalRegions) {
      for (const item of region.items) {
        ...
        rows.push({
          ...
          // 이슈 #136(2026-09-07) 요청: 지역 확인 출원만 보면 3건이 많은지 적은지 가늠이
          // 안 된다 — 그 품목의 전국 검색 건수와 지역이 차지하는 비중을 같이 보여준다.
          // 전국 검색 건수는 지역 귀속 검증 전 후보라 확정 수치와 다른 모집단이다
          // (tradeDisplay 주석과 같은 원칙) — 합산하지 않고 별도 열로만 둔다.
          ...
          // 정책이 육성하기로 지정한 작목인데 지역 확인 출원이 0건이면 최우선 권리화 후보다.
  ...
  const strategyRowsFiltered = useMemo(() => {
    ...
    const searched = query ? strategyRows.filter((row) => row.regionLabel.toLowerCase().includes(query) || row.itemLabel.toLowerCase().includes(query)) : strategyRows;
    const filtered = strategyPolicyOnly ? searched.filter((row) => row.policyTier) : searched;
    const dir = strategySortDir === "asc" ? 1 : -1;
    const numeric = (value: number | null) => value ?? -Infinity;
    return [...filtered].sort((a, b) => {
      let cmp = 0;
      if (strategySortKey === "verdict") cmp = (Number(a.isTopPriority) * 2 + Number(a.isGapAlert)) - (Number(b.isTopPriority) * 2 + Number(b.isGapAlert));
      ...
      else if (strategySortKey === "nationwide") cmp = numeric(a.nationwideCount) - numeric(b.nationwideCount);
      else if (strategySortKey === "nationwideShare") cmp = numeric(a.nationwideShare) - numeric(b.nationwideShare);
  ...
  }, [strategyRows, strategyFilter, strategySortKey, strategySortDir, strategyPolicyOnly]);
...
const STRATEGY_CHIP_LIMIT = 12;
  ...
  // 이슈 #136(2026-09-07): 요약 지도와 지역별 조회가 selectedProvince/selectedMunicipality를
  // 공유해서, 지역별 조회에서 경상남도를 보다가 요약 탭을 누르면 전국 현황 대신 경남
  // 시군구 지도가 그대로 남았다(요약에서 지역을 드릴다운한 뒤 지역별 조회로 갈 때도 같다).
  // 탭 바에서 탭을 직접 고르는 것은 "그 화면을 처음부터 본다"는 뜻이므로 지역 선택과
  // 화면별 검색·필터를 기본값(전국·검색 없음)으로 되돌린다. 카드·지도·랭킹에서 특정
  // 지역을 눌러 들어가는 드릴다운(chooseRegion + setTab)은 선택을 그대로 넘긴다.
  function goPrimaryTab(next: Tab) {
    if (next !== tab) {
  ...
  function handlePrimaryTabsKeyDown(event: React.KeyboardEvent<HTMLElement>) {
    ...
    const currentIndex = Math.max(0, PRIMARY_NAV.findIndex(({ key }) => key === tab || (key === "applications" && EXPLORE_TABS.includes(tab))));
    let nextIndex = currentIndex;
    if (event.key === "ArrowRight") nextIndex = currentIndex >= lastIndex ? 0 : currentIndex + 1;
    else if (event.key === "ArrowLeft") nextIndex = currentIndex <= 0 ? lastIndex : currentIndex - 1;
    else if (event.key === "Home") nextIndex = 0;
    else if (event.key === "End") nextIndex = lastIndex;
    const nextKey = PRIMARY_NAV[nextIndex].key;
  ...
  return <main className="shell">
    ...
    {tab === "summary" && <section role="tabpanel" id="primary-tabpanel-summary" aria-labelledby="primary-tab-summary">
        ...
        {leaderboard.itemCount > 0 && <div className="summary-leaderboard">
          ...
          <div className="leader-grid leader-grid-secondary">
            <article className="leader-card">
              ...
              {leaderboard.conversionHigh.length === 0 ? <p className="empty">누적 출원 20건 이상 품목이 없습니다.</p> : <ol className="leader-list">{leaderboard.conversionHigh.map((row, index) => <li key={row.name}><button type="button" onClick={() => gotoItemDetail(row.name)}><span className="leader-rank">{index + 1}</span><span className="leader-name">{row.name}{row.category && <em className="leader-tag">{row.category.label}</em>}</span><span className="leader-bar"><i style={{ width: `${Math.max(4, row.rate * 100)}%` }} /></span><b className="leader-val">{percent(row.rate)}</b><small className="leader-sub">{number(row.lifeReg)}/{number(row.lifeApp)}</small></button></li>)}</ol>}
            </article>

            <article className="leader-card">
              <div className="leader-card-head"><h4>등록률 하위</h4><span className="leader-card-note">전환이 안 되는 품목</span></div>
              {leaderboard.conversionLow.length === 0 ? <p className="empty">누적 출원 20건 이상 품목이 없습니다.</p> : <ol className="leader-list">{leaderboard.conversionLow.map((row, index) => <li key={row.name}><button type="button" onClick={() => gotoItemDetail(row.name)}><span className="leader-rank">{index + 1}</span><span className="leader-name">{row.name}{row.category && <em className="leader-tag">{row.category.label}</em>}</span><span className="leader-bar leader-bar-low"><i style={{ width: `${Math.max(4, row.rate * 100)}%` }} /></span><b className="leader-val">{percent(row.rate)}</b><small className="leader-sub">{number(row.lifeReg)}/{number(row.lifeApp)}</small></button></li>)}</ol>}
            </article>
          </div>
    ...
    {tab === "applications" && <section className="screen-section coverage-screen" role="tabpanel" id="primary-tabpanel-applications" aria-labelledby="primary-tab-applications">
      ...
      <div className="explore-toolbar">
        <div className="item-search-row">
          ...
          <label className="item-sort-field"><span className="sr-only">정렬 기준</span><select value={regionSort} onChange={(event) => setRegionSort(event.target.value as typeof regionSort)}><option value="gap">공백 많은 순</option>
            ...
            <option value="total">특산품 수순</option>
            <option value="applied">출원 확인순</option>
          </select></label>
        </div>
      </div>
      ...
      {selectedProvince && <section className="coverage-map-card">
        <div className="map-heading"><div><h2>{displayRegionName(selectedProvince)} 시군구 출원율</h2></div><div className="coverage-map-actions"><button className="map-back" type="button" onClick={() => { setSelectedProvince(null); setSelectedMunicipality(null); }}>← 전국</button></div></div>
        ...
        <div className="map-stage coverage-map-stage"><svg className="korea-map coverage-map" viewBox={activeMapViewBox} role="img" aria-label={`${selectedProvince} 시군구별 특산품 출원율 지도`}>{municipalityGeometry ? <>
      ...
      {(() => {
        ...
        const pinned = selectedProvince && comparisonRows.some((row) => row.province === selectedProvince) ? selectedProvince : null;
        const activeProvince = pinned || (selectedCompareProvince && comparisonRows.some((row) => row.province === selectedCompareProvince) ? selectedCompareProvince : comparisonRows[0]?.province || null);
        ...
        return <section className="compare-province-detail-section">
          <div className="compare-province-strip" role="group" aria-label="도 선택" hidden={Boolean(pinned)}>
      ...
      <section className="coverage-directory"><div className="section-heading coverage-directory-heading"><div><span className="coverage-directory-region">{coverageAreaDisplayName}</span><h2>지자체별 특산품 현황</h2></div><span>특산품 {number(coverageListedItemCount)}개 · 출원 확인 {number(coverageArea.applied)}개 · 출원율 {percent(coverageArea.rate)}</span><CsvDownloadButton onClick={() => downloadCsv(`지역별집계_${coverageAreaDisplayName}_${csvDateStamp(dashboardUpdatedAt)}`, ["지역", "특산품 수", "출원 확인", "출원율"], coverageBreakdown.map((row) => [displayRegionName(row.label), row.coverage.total, row.coverage.applied, percent(row.coverage.rate)]))} /></div>
        ...
        {(() => {
          ...
          if (!selectedProvince) {
            const rows = sortCoverageRows(key ? coverageBreakdown.filter(matchesQuery) : coverageBreakdown);
          ...
          // 이슈 #136(2026-09-07): 품목별 조회와 같은 마스터-디테일로 맞춘다 — 왼쪽은 늘
          // 보이는 지자체 스크롤 목록, 오른쪽은 고른 지자체 하나의 특산품 목록. 도 단위
          // 시군구 미지정 행("경기도" 자체)은 시군구와 급이 다르므로 "도 전체" 자리에서
          // 보여준다. 목록을 <details>로 접어 두면 시군구를 고른 뒤 목록이 사라져 다른
          // 시군구로 옮겨갈 수 없었다.
          const rows = key ? provinceCoverageRows.filter(matchesQuery) : provinceCoverageRows;
          if (rows.length === 0) return <p className="empty">&ldquo;{regionQuery}&rdquo; 검색 결과가 없습니다.</p>;
          ...
          const selectedRow = selectedMunicipality ? municipalityRows.find((row) => row.label === selectedMunicipality) || null : null;
          const provinceWide = specialtyCoverage(snapshot.regions.filter((region) => region.sido === selectedProvince));
          return <div className="item-explorer coverage-explorer">
            <aside className="item-list-panel">
              <div className="item-list-head"><strong>{displayRegionName(selectedProvince)} 지자체</strong><span>시군구 {municipalityRows.length}곳</span></div>
              <ul className="item-list">
                <li><button type="button" className={selectedRow ? "" : "active"} title={`${displayRegionName(selectedProvince)} 전체 합계`} onClick={() => setSelectedMunicipality(null)}>
                  <span className="item-list-name">{displayRegionName(selectedProvince)} 전체</span>
                  <b>{percent(provinceWide.rate)}</b>
                </button></li>
                {municipalityRows.map((row) => <li key={row.key}>
                  <button type="button" className={selectedRow?.key === row.key ? "active" : ""} title={`특산품 ${number(row.coverage.total)}개 · 출원 확인 ${number(row.coverage.applied)}개`} onClick={() => openMunicipality(row.label, { scroll: false })}>
                    <span className="item-list-name">{displayRegionName(row.label)}</span>
                    <b>{percent(row.coverage.rate)}</b>
                  </button>
                </li>)}
                {municipalityRows.length === 0 && <li className="empty">시군구 단위로 구분된 원본 자료가 없습니다.</li>}
              </ul>
            </aside>
            <div className="item-detail-panel coverage-detail-panel">
              {selectedRow ? coverageCard(selectedRow) : <>
                {unclassifiedRows.length > 0 && <div className="coverage-unclassified-grid">{unclassifiedRows.map((row) => <article className="coverage-region-card unclassified" key={row.key}><div className="coverage-region-head"><div><strong>{displayRegionName(row.label)} 전체(시군구 미지정)</strong><small>특산품 {number(row.coverage.total)}개</small></div><div className="coverage-region-summary">{coverageGap(row.coverage) > 0 && <span className="coverage-gap-flag" title="수집된 특산품 중 출원이 확인되지 않은 수">공백 {number(coverageGap(row.coverage))}개</span>}<span>출원 확인 특산품 {number(row.coverage.applied)}개</span><b>{percent(row.coverage.rate)}</b></div></div><div className="coverage-specialty-list">{row.items.map(({ region, item, label }) => { const status = specialtyFilingStatus(item); return <button type="button" key={`${regionKey(region)}-${item.specialtyId}`} onClick={() => { chooseRegion(region); setSelectedItemId(item.specialtyId || ""); setTab("regions"); }}><span>{label}{item.regionalSpecialtyCropBadge && <em className={`crop-badge crop-badge-${item.regionalSpecialtyCropBadge.tier}`}>{item.regionalSpecialtyCropBadge.tier}</em>}</span><small className={`specialty-status ${status.filed ? "filed" : "unfiled"}`}>{status.label}</small></button>; })}</div></article>)}</div>}
                <p className="empty">왼쪽 목록에서 시군구를 고르면 그 지역 특산품 목록이 열립니다.</p>
              </>}
            </div>
          </div>;
        })()}
      ...
      <div className="explore-toolbar">
        <div className="item-search-row">
          ...
          <label className="item-sort-field"><span className="sr-only">정렬 기준</span><select value={itemSort} onChange={(event) => setItemSort(event.target.value as typeof itemSort)}>
            <option value="gap">공백 지역 많은 순</option>
            ...
            <ul className="item-list">
              {visibleItemRows.map((row) => { const decidedRegions = row.availableRegions.length; const spark = sparklinePoints(row.matchedItems); return <li key={row.name}>
                <button type="button" className={selectedItemRow?.name === row.name ? "active" : ""} title={`${row.category ? `${row.category.label} · ` : ""}${row.regions.length}개 지역`} onClick={() => selectItemAndScroll(row.name)}>
                  ...
                  <b>{decidedRegions
                    ? ((itemSort === "gap" || itemGapOnly || !row.trademarks)
                      ? <span className="item-list-gap">공백 {number(itemGapRegions(row))}개 지역</span>
                      : `${number(row.trademarks)}건`)
                    : "집계 대기"}</b>
                </button>
              </li>; })}
          ...
          <div className="item-detail-panel">{selectedItemRow ? (() => { const row = selectedItemRow; const decidedRegions = row.availableRegions.length; const pendingRegions = Math.max(0, row.regions.length - decidedRegions); const nationwideOnly = Math.max(0, row.trademarksDisplay - row.trademarks); const registrationRate = decidedRegions && row.trademarks ? row.registered / row.trademarks : null;
            ...
            {(() => {
              ...
              return <section className="region-trademark-panel">
                <div className="region-trademark-head">
                  <div><strong>{displayRegionName(itemRegionPick)} · {row.name}</strong><small>지역 확인 출원 {number(local)}건 · 등록 {number(reg)}건{local ? ` · 등록률 ${percent(reg / local)}` : ""}</small></div>
                  <button type="button" className="region-trademark-close" onClick={() => setItemRegionPick("")}>닫기</button>
                </div>
                {examples.length ? <ul className="region-trademark-list">{examples.slice(0, 12).map((example, index) => <li key={example.applicationNumber || index}>
                  <span className="region-trademark-name">{example.title || "상표명 미기록"}</span>
                  <span className={`region-trademark-status ${example.statusCategory === "registered" ? "registered" : "pending"}`}>{example.applicationStatus || "상태 미기록"}</span>
            ...
            {/* 컨설팅 보고서 Ⅲ장 6)항의 재료라 추이·비중보다 먼저 읽혀야 한다(2026-09-08). */}
            ...
            {decidedRegions > 0 && <>
              <RegionTrend region={{ region: row.name, items: row.matchedItems }} heading="연도별 출원·등록 추이" subtitle={`${row.name} · 전체 지역 합계`} prominent adjustable emptyLabel="이 품목은 아직 연도별 데이터가 없습니다." />
    ...
    {tab === "strategy" && <section className="screen-section strategy-screen" role="tabpanel" id="primary-tabpanel-applications" aria-labelledby="primary-tab-applications">
      <div className="section-heading"><div><h2>품목별 비즈니스 확장 경로</h2></div><span>전국 상표DB 기준 · 지역 한정 아님</span></div>
      <p className="screen-note">품목을 고르면 그 품목의 전국 상표 활동을 보여줍니다. 특정 지역의 현황이 아니라 전체 상표DB에서 탐색한 결과입니다. 전국 흐름 배치가 반영된 품목은 <strong>원물 → 가공품 → 서비스</strong> 단계별 <strong>지정상품</strong>과 확장 방향 제안까지 함께 나옵니다.</p>
      ...
      {strategyFlowRows.length === 0
        ...
        : <>
          ...
          {strategySelectedFlow ? <div className="strategy-flow-detail">
            ...
            <RegionTrend region={{ region: strategySelectedFlow.name, items: strategySelectedFlow.matchedItems }} heading="연도별 출원·등록 추이" subtitle={`${strategySelectedFlow.name} · 전체 지역 합계`} prominent emptyLabel="이 품목은 아직 연도별 데이터가 없습니다." />
      ...
      <summary><span>지역×품목 공백 알림 표 전체 보기</span><small>{number(strategyRows.length)}건</small></summary>
      <p className="screen-note">지역×품목 단위 비즈니스 확장 전략 브리핑입니다. 표에서 행을 고르면 오른쪽에 브리핑과 근거가 열립니다. <b>지역 확인 출원</b>은 출원인 주소가 그 지역과 일치한 확정 건수, <b>전국 검색</b>은 같은 품목명의 전국 검색 결과(지역 확인 전 후보)로 서로 다른 모집단이며, <b>전국 대비</b>는 두 값의 비율입니다.</p>
        ...
        <div className="strategy-table-layout">
          <div className="strategy-table-wrap">
            <table className="strategy-table">
              <thead><tr>
                {([["region", "지역"], ["item", "품목"], ["trademark", "지역 확인 출원"], ["nationwide", "전국 검색"], ["nationwideShare", "전국 대비"], ["registration", "등록률"], ["share", "지역 출원인 비중"], ["verdict", "판정"]] as const).map(([key, label]) => <th key={key} aria-sort={strategySortKey !== key ? "none" : strategySortDir === "asc" ? "ascending" : "descending"}>
              ...
              <tbody>
                {strategyRowsFiltered.length === 0 && <tr><td colSpan={8} className="empty">검색 결과가 없습니다.</td></tr>}
                ...
                >
                  ...
                  <td>{row.itemLabel}{row.policyTier && <em className={`crop-badge crop-badge-${row.policyTier}`} title="농촌진흥청 지역특화작목">{row.policyTier}</em>}</td>
                  <td>{row.uniqueTrademarkCount !== null ? `${number(row.uniqueTrademarkCount)}건` : "—"}</td>
                  <td className="strategy-nationwide" title={row.nationwideCapped ? "검색 상한(1,800건)에 걸려 실제 전국 건수는 이보다 많습니다" : undefined}>{nationwideCountLabel(row.nationwideCount, row.nationwideCapped)}</td>
          ...
          <aside className="strategy-detail">
            {strategySelectedRow && !strategySelectedRow.item.briefing ? <div className="strategy-no-briefing">
              <strong>{strategySelectedRow.regionLabel} · {strategySelectedRow.itemLabel}</strong>
              <p>정책이 육성하기로 지정한 작목인데 지역 주소가 확인된 상표 출원이 <b>0건</b>입니다. 자동 생성 브리핑은 출원 실적이 있는 품목에만 만들어지므로 아직 없습니다 — 권리화를 먼저 검토할 대상입니다.</p>
              <button type="button" className="strategy-jump-link" onClick={() => { chooseRegion(strategySelectedRow.region); setSelectedItemId(strategySelectedRow.item.specialtyId || ""); setTab("regions"); }}>지자체별 조회에서 자세히 보기 →</button>
            </div> : strategySelectedRow && strategySelectedRow.item.briefing ? <>
              <BusinessStrategyCard
                ...
                nationwideCount={strategySelectedRow.nationwideCount}
                nationwideShare={strategySelectedRow.nationwideShare}
                nationwideCapped={strategySelectedRow.nationwideCapped}
    ...
    {tab === "data" && pipeline && <section className="screen-section data-overview" role="tabpanel" id="primary-tabpanel-data" aria-labelledby="primary-tab-data">
...
function RegionDetail({ region, item, onItem, verifiedExamples }: { region: Region; item: Item | undefined; onItem: (id: string) => void; verifiedExamples: TrademarkExample[] }) {
  ...
  // 안 거친다 — 화면에 그대로 노출하면 같은 지역이 다른 화면 표기와 달라 보인다.
  const displayName = displayRegionName(region.region);
  const heading = <div className="detail-heading"><div><h2>{displayName}</h2><p>법정동코드 {region.regionCode || "미확정"}</p></div><span className={`state state-${region.dataState}`}>{STATE_LABELS[region.dataState] || region.dataState}</span></div>;
  if (!item) {
    return <div className="detail-panel">
      {heading}
      <div className="item-tabs" role="tablist" aria-label={`${displayName} 특산품`} />
  ...
  return <div className="detail-panel">
    ...
    <div className="metric-reading-note"><strong>출원 건수 기준</strong><p><b>{region.sigungu || displayName} {itemName(item)} 출원</b>은 출원인 주소가 {displayName}으로 확인된 고유 출원 수입니다. 전국 검색 후보나 주소가 확인되지 않은 출원은 포함하지 않습니다.</p></div>
