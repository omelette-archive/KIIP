...
function RegionDetail({ region, item, onItem }: { region: Region; item: Item; onItem: (id: string) => void }) {
  ...
  return <div className="detail-panel">
    ...
    <div className="detail-grid">
      <article><span>{itemName(item)} · 지역 주소 일치 출원</span><strong>{regionalAvailable ? `${number(localCount)}건` : trade.value !== null ? `${number(trade.value)}건 (잠정)` : "지역별 집계 대기"}</strong><small>{regionalAvailable ? `출원인 주소가 ${region.region}으로 확인된 고유 출원` : `전국 검색 잠정치 · ${pendingReason}`}</small></article>
