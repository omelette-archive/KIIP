...
export default function Dashboard({ snapshot, geometry }: { snapshot: Snapshot; geometry: MapGeometry }) {
  ...
  return <main className="shell">
    ...
    {tab === "summary" && <>
      <section className="hero"><div><h1>지역 특산품 상표 분석</h1><p className="hero-copy">지역별 특산품과 관련 상표 현황을 한눈에 확인합니다.</p></div><div className="hero-note"><span>조사 범위</span><strong>{snapshot.coverage.observedRegionCount}개 지역 · {snapshot.coverage.regionItemCount}건의 지역-품목 기록</strong><p>지금까지 확인한 데이터 범위이며, 계속 늘어납니다.</p></div></section>
      ...
      <section className="map-workspace">
        ...
        <aside className="map-insight"><h2>{displayRegionName(selectedMunicipality || selectedProvince || "전국")}</h2><p className="insight-summary">확인 특산품 {number(visibleSpecialtyCoverage.total)}개 중 출원 확인 {number(visibleSpecialtyCoverage.applied)}개 · 출원율 {percent(visibleSpecialtyCoverage.rate)} · 판정 완료 {number(visibleSpecialtyCoverage.decided)}개{visibleSpecialtyCoverage.pending ? ` · 집계 대기 ${number(visibleSpecialtyCoverage.pending)}개` : ""}</p><div className="mini-list">{visibleItems.slice(0, 5).map(({ region, item, label }) => <button type="button" key={`${regionKey(region)}-${item.specialtyId}`} onClick={() => { chooseRegion(region); setSelectedItemId(item.specialtyId || ""); setTab("regions"); }}><span><strong>{region.sigungu || region.region} / {label}</strong><small>{noticeBasis(item)} · NICE {item.niceClass}류</small></span><b>{item.metrics.uniqueTrademarkCount.availability === "available" ? (item.metrics.uniqueTrademarkCount.value || 0) > 0 ? `출원 확인 · ${number(item.metrics.uniqueTrademarkCount.value)}건` : "출원 없음 · 판정 완료" : "지역별 집계 대기"}</b></button>)}{visibleItems.length === 0 && <p className="empty">이 지역에는 고시명칭이 확인된 특산품이 없습니다.</p>}</div><div className="insight-note"><strong>출원율 계산</strong><p>출원 확인 특산품 수 ÷ 이 지역의 확인 특산품 전체 수입니다(2026-08-20 계산 방식 변경). 판정 완료·집계 대기 수를 함께 표시하며, 아직 집계가 안 끝난 대기 품목은 출원 미확인으로 계산합니다 — 데이터가 쌓일수록 값이 올라갈 수 있습니다.</p></div></aside>
      </section>
    ...
    {tab === "applications" && <section className="screen-section coverage-screen">
      <div className="screen-heading"><div><h1>지역별 특산품 출원율</h1></div><p>지도 색과 숫자로 시도별 출원율을 비교하고, 시도를 누르면 시군구까지 내려갑니다.</p></div>
      <section className="coverage-kpis" aria-label={`${coverageAreaDisplayName} 특산품 출원 현황`}>
        ...
        <article><span>출원 확인 특산품</span><strong>{number(coverageArea.applied)}개</strong><small>확인 특산품 전체 {number(coverageArea.total)}개 중 1건 이상 출원 확인</small></article>
        <article className="coverage-rate-kpi"><span>특산품 출원율</span><strong>{percent(coverageArea.rate)}</strong><small>전체 {number(coverageArea.total)}개 중 출원 확인 {number(coverageArea.applied)}개{coverageArea.pending ? ` · 집계 대기 ${number(coverageArea.pending)}개` : " · 전체 판정 완료"}</small></article>
      </section>
      <section className="coverage-map-card">
        ...
        <p className="map-metric-description"><strong>특산품 출원율</strong><span>출원 확인 특산품 수 ÷ 확인 특산품 전체 수 · 집계 대기는 출원 미확인으로 계산해 분모에 포함합니다.</span></p>
      ...
      <section className="coverage-directory"><div className="section-heading"><div><h2>{coverageAreaDisplayName} 특산품 목록</h2></div><span>확인 {number(coverageArea.total)}개 · 출원 확인 {number(coverageArea.applied)}개 · 출원율 {percent(coverageArea.rate)}</span></div>
        <div className="coverage-region-grid">{coverageBreakdown.map((row) => <article className={selectedMunicipality && row.label === selectedMunicipality ? "coverage-region-card selected" : "coverage-region-card"} key={row.key}><div className="coverage-region-head"><div><strong>{displayRegionName(row.label)}</strong><small>전체 특산품 {number(row.coverage.total)}개 중 출원 확인 {number(row.coverage.applied)}개 · 판정 완료 {number(row.coverage.decided)}개{row.coverage.pending ? ` · 대기 ${number(row.coverage.pending)}개` : ""}</small></div><b>{percent(row.coverage.rate)}</b>{!selectedProvince && <button type="button" onClick={() => openProvince(row.label)}>지도에서 보기</button>}</div><div className="coverage-specialty-list">{row.items.map(({ region, item, label }) => <button type="button" key={`${regionKey(region)}-${item.specialtyId}`} onClick={() => { chooseRegion(region); setSelectedItemId(item.specialtyId || ""); setTab("regions"); }}><span>{selectedProvince ? label : `${region.sigungu || region.region} / ${label}`}</span><small>{item.metrics.uniqueTrademarkCount.availability === "available" ? (item.metrics.uniqueTrademarkCount.value || 0) > 0 ? `출원 확인 · ${number(item.metrics.uniqueTrademarkCount.value)}건` : "출원 없음 · 판정 완료" : "집계 대기"}</small></button>)}</div></article>)}</div>
      </section>
    </section>}

    {tab === "regions" && <section className="screen-section"><div className="screen-heading"><div><h1>지자체별 조회</h1></div><p>지역 → 품목 → 근거 지표 순으로 확인합니다.</p></div><section className="workspace" aria-label="지역별 상세 조회"><aside className="region-panel"><div className="panel-heading"><div><h2>수집 지역</h2></div><span>{filteredRegions.length}건</span></div><label className="search-field"><span className="sr-only">지역 또는 품목 검색</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="지역 또는 품목 검색" /></label><div className="region-list">{filteredRegions.map((region) => { const count = regionTrademarkValue(region); const coverage = specialtyCoverage([region]); return <button type="button" key={regionKey(region)} className={regionKey(selectedRegion) === regionKey(region) ? "region-button active" : "region-button"} onClick={() => chooseRegion(region)}><span><strong>{region.region}</strong><small>{coverage.total}개 확인 특산품 · 출원율 {percent(coverage.rate)} (출원 확인 {coverage.applied}/{coverage.total}) · 판정 완료 {coverage.decided}개{coverage.pending ? ` · ${coverage.pending}개 대기` : ""}<br />{count === null ? "지역별 출원 집계 대기" : `주소 일치 출원 ${number(count)}건`}</small></span><span className={`state state-${region.dataState}`}>{STATE_LABELS[region.dataState] || region.dataState}</span></button>; })}{filteredRegions.length === 0 && <p className="empty">검색 결과가 없습니다.</p>}</div></aside><RegionDetail region={selectedRegion} item={selectedItem} onItem={setSelectedItemId} /></section></section>}

    {tab === "items" && <section className="screen-section">
      <div className="screen-heading"><div><h1>품목별 조회</h1></div><p>품목마다 확인 지역과 상표 현황을 카드 한 장에 요약했습니다.</p></div>
    ...
    {tab === "gaps" && <section className="screen-section">
      <div className="screen-heading"><div><h1>미출원 특산품</h1></div><p>실제로 존재하는 특산품인데, 그 지역 주소로 낸 상표 출원이 KIPRIS에 한 건도 없는 경우입니다.</p></div>
    ...
    {tab === "compare" && <section className="screen-section">
      ...
      <section className="compare-region-section"><div className="compare-section-head"><div><span>현재 확인 가능</span><h2>지역별 특산품 출원 현황</h2></div><p>정책 비교 결과가 아니라, 비교에 투입될 현재 데이터입니다.</p></div><div className="compare-region-table"><div className="compare-region-head"><span>지역</span><span>전체 확인 특산품</span><span>전체 특산품 출원율</span><span>정책 비교</span></div>{comparisonRows.map(({ province, coverage, names }) => <div className="compare-region-row" key={province}><strong>{province}</strong><div><b>{number(coverage.total)}개</b></div><div><b>{percent(coverage.rate)}</b><small>전체 {coverage.total}개 중 출원 확인 {coverage.applied}개 · 판정 완료 {coverage.decided}개{coverage.pending ? ` · ${coverage.pending}개 대기` : ""}</small></div><span className="compare-waiting">원본 대기</span><details className="compare-items-detail"><summary>확인된 특산품 {number(names.length)}개 보기</summary><div className="compare-item-chips">{names.map((name) => <span key={name}>{name}</span>)}</div></details></div>)}</div></section>
