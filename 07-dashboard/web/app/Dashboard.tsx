...
export default function Dashboard({ snapshot, geometry }: { snapshot: Snapshot; geometry: MapGeometry }) {
  ...
  return <main className="shell">
    ...
    {tab === "items" && <section className="screen-section"><div className="screen-heading"><div><p className="eyebrow">ITEM EXPLORER</p><h1>품목별 조회</h1></div><p>품목을 기준으로 지역과 상표 활동을 다시 묶었습니다.</p></div><div className="item-screen"><div className="item-screen-toolbar"><label><span className="sr-only">품목 검색</span><input value={itemQuery} onChange={(event) => setItemQuery(event.target.value)} placeholder="품목 또는 지역 검색" /></label><span>{itemRows.length}개 품목</span></div><div className="item-table" role="table"><div className="item-table-head" role="row"><span>품목</span><span>수집 지역</span><span>지역 고유 상표</span><span>지역 등록 상표</span><span>등록률</span></div>{itemRows.map((row, index) => <div className="item-table-row" role="row" key={row.name}><span><b>{String(index + 1).padStart(2, "0")}</b><strong>{row.name}</strong></span><span>{row.regions.join(", ")}</span><span>{row.verified ? `${number(row.trademarks)}건` : "검증 중"}</span><span>{row.verified ? `${number(row.registered)}건` : "검증 중"}</span><span>{row.verified ? percent(row.trademarks ? row.registered / row.trademarks : null) : "—"}</span></div>)}</div><div className="method-note"><strong>집계 기준</strong><p>고시명칭·NICE류가 확정된 품목만 공식 명칭 기준으로 재그룹합니다(&quot;풋고추&quot;·&quot;파프리카&quot;는 같은 &quot;고추&quot;로 합쳐짐). 아직 고시명칭이 확정되지 않은 검토대기 원물명은 지역별 상세 화면에서 원문 그대로 확인할 수 있습니다.</p></div></div></section>}
