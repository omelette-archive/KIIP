...
//
// 근거는 오직 스냅샷에 실린 지정상품이다. businessFlow(원물→가공품→서비스 배치)는 아직
// 스냅샷에 0건이라 그 위에 세우면 전 품목이 빈칸이 된다. 대신 출원의 지정상품 상품류를
// 쓴다 — "이 품목이 실제로 도달한 류"와 "같은 유형의 다른 특산품은 도달했는데 이 품목은
// 비어 있는 류"의 차이가 곧 확장 여지다. 없는 걸 지어내지 않는다.
// standalone-client.js의 expansionIndex/expansionReportHtml과 같은 규칙.
...
function expansionSuggestions(flow: NationwideFlow, opts: { surging?: boolean } = {}): { kind: string; text: string }[] {
  ...
  // 이슈 #110: "이 지역이 산지다"라는 지역 클러스터 주장은 원물 단계 상위 출원인이
  // 생산 주체형으로 확인된 품목(hasRegionalSignal)에만 붙인다. topRegions 배열 자체는
  // 근사치 참고값으로 전 품목에 실리지만(#119), 그걸 "산지"로 단정하는 문장은 다르다 —
  // uncertain 품목에서는 대기업 본사 주소(예: 블루베리 원물 1위 = 서울 강남구)가 잡힌다.
...
export default function Dashboard({ snapshot, geometry, registrationExamples }: { snapshot: Snapshot; geometry: MapGeometry; registrationExamples: VerifiedRegistrationExamples }) {
  ...
  return <main className="shell">
        ...
        {leaderboard.itemCount > 0 && <div className="summary-leaderboard">
          ...
          <div className="leader-grid leader-grid-primary">
            ...
            <article className="leader-card">
              ...
              {leaderboard.topItems.length === 0 ? <p className="empty">해당 기간 집계가 없습니다.</p> : <ol className="leader-list">{leaderboard.topItems.map((row, index) => { const value = leaderMetric === "application" ? row.app : row.reg; const top = leaderMetric === "application" ? leaderboard.topItems[0].app : leaderboard.topItems[0].reg; return <li key={row.name}><button type="button" onClick={() => gotoItemDetail(row.name)}><span className="leader-rank">{index + 1}</span><span className="leader-name">{row.name}{row.category && <em className="leader-tag">{row.category.label}</em>}</span><b className="leader-val">{number(value)}</b><span className="leader-bar"><i style={{ width: `${top ? Math.max(3, value / top * 100) : 0}%` }} /></span></button></li>; })}</ol>}
            </article>

            <article className="leader-card">
              <div className="leader-card-head"><h4>{leaderMetric === "application" ? "출원" : "등록"} 많은 유형</h4><span className="leader-card-note">비중</span></div>
              {leaderboard.topCategories.length === 0 ? <p className="empty">해당 기간 집계가 없습니다.</p> : <ol className="leader-list">{leaderboard.topCategories.slice(0, LEADER_LIMIT).map((row, index) => { const value = leaderMetric === "application" ? row.app : row.reg; const share = leaderboard.categoryTotal ? value / leaderboard.categoryTotal : 0; return <li key={row.label}><button type="button" title={`비중 ${percent(share)}`} onClick={() => gotoCategory(row.code)}><span className="leader-rank">{index + 1}</span><span className="leader-name">{row.label}</span><b className="leader-val">{number(value)}</b><span className="leader-bar"><i style={{ width: `${Math.max(3, share * 100)}%`, background: categoryShareColor(row.label) }} /></span></button></li>; })}</ol>}
            </article>
          </div>
          ...
          <div className="leader-grid leader-grid-secondary">
            <article className="leader-card">
              ...
              {leaderboard.conversionHigh.length === 0 ? <p className="empty">누적 출원 20건 이상 품목이 없습니다.</p> : <ol className="leader-list">{leaderboard.conversionHigh.map((row, index) => <li key={row.name}><button type="button" title={`등록 ${number(row.lifeReg)}건 / 출원 ${number(row.lifeApp)}건`} onClick={() => gotoItemDetail(row.name)}><span className="leader-rank">{index + 1}</span><span className="leader-name">{row.name}{row.category && <em className="leader-tag">{row.category.label}</em>}</span><b className="leader-val">{percent(row.rate)}</b><span className="leader-bar"><i style={{ width: `${Math.max(3, row.rate * 100)}%` }} /></span></button></li>)}</ol>}
            </article>

            <article className="leader-card">
              <div className="leader-card-head"><h4>등록률 하위</h4><span className="leader-card-note">전환이 안 되는 품목</span></div>
              {leaderboard.conversionLow.length === 0 ? <p className="empty">누적 출원 20건 이상 품목이 없습니다.</p> : <ol className="leader-list">{leaderboard.conversionLow.map((row, index) => <li key={row.name}><button type="button" title={`등록 ${number(row.lifeReg)}건 / 출원 ${number(row.lifeApp)}건`} onClick={() => gotoItemDetail(row.name)}><span className="leader-rank">{index + 1}</span><span className="leader-name">{row.name}{row.category && <em className="leader-tag">{row.category.label}</em>}</span><b className="leader-val">{percent(row.rate)}</b><span className="leader-bar leader-bar-low"><i style={{ width: `${Math.max(3, row.rate * 100)}%` }} /></span></button></li>)}</ol>}
            </article>
          </div>
    ...
    {tab === "compare" && <section className="screen-section" role="tabpanel" id="primary-tabpanel-compare" aria-labelledby="primary-tab-compare">
      ...
      {/* 사용자 요청(2026-09-08): "공식 원본 반영 완료"·수집 방침 문구는 개발쪽 톤이라 배포에
          안 어울린다 → 제거하고 9·18·42 숫자를 더 잘 보이게. */}
      <div className="compare-tier-tiles">
        {([["대표작목", 9], ["집중육성작목", 18], ["자체육성작목", 42]] as const).map(([tier, count]) => <article key={tier} className={`compare-tier-tile crop-badge-${tier}`}><span>{tier}</span><strong>{count}</strong></article>)}
      </div>
