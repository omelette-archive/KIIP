...
function calloutLabels(shapes: (ProvinceShape | MunicipalityShape)[], viewBox: string, valueFor: (name: string) => number | null, labelFor: (value: number | null) => string, includeName: (name: string) => boolean = () => true, inlineByDefault = true): MapLabel[] {
  const { x, y, width, height } = parseViewBox(viewBox);
  ...
  const inlineRows = inlineByDefault ? rows.slice().sort((a, b) => a.shape.labelY - b.shape.labelY).filter((row, index, sorted) => {
    const overlaps = sorted.slice(0, index).some((other) => Math.abs(row.shape.labelX - other.shape.labelX) < 52 && Math.abs(row.shape.labelY - other.shape.labelY) < 24);
    if (overlaps) connectorRows.push(row);
    return !overlaps;
  }) : [];
