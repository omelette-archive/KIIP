...
const { runBibliographyGoodsTests } = require("./bibliographyGoodsSelftest");
const {
  parseCsvLine,
  readNormalizedCsv,
  makeBatchQuery,
...
async function run() {
  ...
  await runBibliographyGoodsTests();
...
<response>
  ...
  console.log("12-3) 출원번호 기반 상표 출원인 주소 — 응답 파싱·영속 누적");
  {
    ...
    // 2026-09-09(사용자): 목록이 농업 쪽으로만 채워져 있어 수산 산지 주체가 빠져 있었다.
    // 법정 생산자 조직 세 가지를 넣었고, 일반 회사명은 여전히 걸리지 않아야 한다.
