...
"use strict";
/**
 * #73 경로 B — 등록번호 기반(ip-registry-cache.json) 출원인 주소 미확인
 * (unmatched/ambiguous) 건만 선별해 등록원부를 다시 조회하고, 새 시도·시군구
 * 별칭 규칙(#72)을 적용한 결과를 기준 캐시와 분리된 별도 캐시에 남긴다. 기준
 * 캐시는 절대 직접 수정하지 않는다 — 검증 후 사람이 --merged-out으로 명시적으로
 * 병합 결과를 만들어 확인한 뒤 교체한다.
 *
 * 경로 A(03-match-trademarks/refreshUnverifiedApplicantRegions.js)와 구조를
 * 맞췄다. 자세한 정책은 docs/applicant-region-recovery-runbook.md 참고.
 */
...
const { sanitizeRegistryRecordForCache, isRateLimitError, registryNumbers } = require("./lib/ipRegistryEnricher");
const { loadCache, saveCache } = require("./lib/ipRegistryCache");
const { buildRegistryRefreshManifest, classifyRegistryCacheEntry } = require("./lib/applicantRegionRefresh");
const {
  loadBudgetState,
  saveBudgetState,
  isResumeBlocked,
...
function usage(message) {
  ...
  console.error(
    [
      "사용법:",
      "  node 03-match-trademarks/refreshUnverifiedRegistryRegions.js --cache <기준 캐시> [옵션]",
      "",
      "옵션:",
      "  --input <path>          ③ 검색 결과 JSON — 주면 캐시에 아예 없는 등록번호도 not_collected로 집계",
      "  --dry-run                API 호출 없이 재조회 후보 manifest만 생성",
      "  --manifest-out <path>    manifest JSON 경로 (기본: <cache 옆> registry-refresh-manifest.json)",
...
async function main() {
  ...
  const universe = document ? registryNumbers(document) : undefined;
  const manifest = buildRegistryRefreshManifest(baseCache, { registrationNumbers: universe });

  const manifestOutPath = path.resolve(
    args["manifest-out"] || path.join(path.dirname(cachePath), "registry-refresh-manifest.json")
  );
  writeJson(manifestOutPath, manifest);
  console.error(
    `[refreshRegistry] manifest: total=${manifest.totalRowCount}, ` +
  ...
  const refreshCachePath = path.resolve(
    args["refresh-cache"] || path.join(path.dirname(cachePath), "ip-registry-refresh-cache.json")
  );
  ...
  const budgetStatePath = path.resolve(
    args["budget-state"] || path.join(path.dirname(cachePath), "registry-refresh-daily-budget.json")
  );
  ...
  if (blocked) {
    console.error(
      `[refreshRegistry] 이전 429 이후 재개 대기 중 - resumeNotBefore=${budgetState.resumeNotBefore}. 새 호출 없이 종료합니다.`
    );
  } else if (dailyBudget !== undefined && effectiveLimit < limit) {
    console.error(
      `[refreshRegistry] 일일 예산(${dailyBudget}) 중 ${budgetState.callsUsed}건 사용 - 이번 실행은 ${effectiveLimit}건만 호출합니다.`
    );
  }
  ...
  console.error(
    `[refreshRegistry] 재조회 후보 ${manifest.refreshCandidateCount}건 중 미처리 ${pending.length}건, ` +
    ...
    while (cursor < selected.length) {
      ...
      try {
        ...
        if (completedThisRun % checkpointEvery === 0) {
          ...
          console.error(`[refreshRegistry] checkpoint new=${completedThisRun}, cache=${refreshEntries.size}`);
        }
      } catch (error) {
  ...
  const report = {
    ...
    beforeAfter: selected
      ...
      .map((number) => ({
        ...
        before: classifyRegistryCacheEntry(number, baseCache.get(number)).category,
        after: classifyRegistryCacheEntry(number, refreshEntries.get(number)).category,
      })),
  };
  ...
  const reportOutPath = path.resolve(
    args["report-out"] || path.join(path.dirname(cachePath), "registry-refresh-report.json")
  );
  writeJson(reportOutPath, report);
  console.error(
    `[refreshRegistry] 완료: 요청 ${report.requested}, 신규 완료 ${report.newlyComplete}, ` +
  ...
  if (args["merged-out"]) {
    ...
    for (const [number, entry] of refreshEntries) {
      const before = classifyRegistryCacheEntry(number, baseCache.get(number)).category;
      const after = classifyRegistryCacheEntry(number, entry).category;
    ...
    console.error(
      `[refreshRegistry] 병합 결과(개선된 ${mergedCount}건만 반영) -> ${mergedOutPath} ` +
...
if (require.main === module) {
  main().catch((error) => {
    console.error(`[refreshRegistry] 실패: ${error.message}`);
    process.exit(1);
  });
}
