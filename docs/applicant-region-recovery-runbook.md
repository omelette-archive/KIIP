...
4. 복수 출원인이 모두 같은 결과일 때 그 결과를 사용한다. 지역이 서로 다르면
   ...
   2. 그 외 **공동출원인 중 누구든 하나라도 `inside`**면 `coapplicant_inside`로 인정한다.
      다만 **원물 특산품은 이 완화를 적용하지 않고 1번(생산자 주체형)에서 멈춘다**
      (2026-09-09 사용자 기준 — "협동조합은 원물일 경우에만 공동출원인 인정해주고,
      가공품인 특산품의 경우 일반 기업 등 모두 가능해"). 원물은 산지 귀속이 지표의
      핵심이라 유통·판매 기업 본사 지역에 얹히면 안 되고, 가공품은 기업이 가공·판매
      주체이므로 기업 소재지도 정당한 귀속처다.
      - 원물/가공품 판정은 `02-normalize-items/lib/derivedRawItems.js`의
        `specialtyStageOf(고시명칭)`이다. **가공 표지가 없으면 원물**이 기본값이고,
        가공 표지는 셋이다 — 통짜 가공품명(`WHOLE_ITEM_NAMES`: 고추장·막걸리 등),
        가공 접미어(`PROCESSED_SUFFIXES`: `~차`·`~장`·`~주` 등), 이름 어디에 있어도
        가공을 뜻하는 낱말(`PROCESSED_MARKERS`: 가공·절임·발효·세트 등).
      - 처음에는 「쌀」·「소고기」·「굴」처럼 수식어도 접미어도 없는 이름을 판정 불가로
        두고 완화 쪽으로 넘겼으나, 2026-09-10 사용자 지적으로 뒤집었다 — "쌀도 원물이라고
        봐야지, 탈곡 전의 쌀은 지정상품으로 안 쓰니." 지정상품에 오르는 이름은 이미
        유통 형태를 전제한다(벼가 아니라 쌀, 소가 아니라 소고기).
      - `PROCESSED_MARKERS`는 실제 카탈로그 고시명칭 792개 중 접미어로 안 걸린 것을
        훑어 오탐 없이 걸러지는 낱말만 고른 목록이다(「죽순」·「참죽나무」가 걸리지 않도록
        한 글자는 넣지 않는다). **카탈로그가 늘면 다시 훑어야 한다.**
      - `null`은 이름이 비었을 때만 나오고, 이때는 완화 쪽이다 — 품목명을 못 넘긴
        호출부가 조용히 엄격해지지 않게 한다.
      - 완화가 적용될 때 같은 상표가 여러 지역에서 집계되는 것은 의도된 더블 카운트이며,
        지역 안 집계는 출원번호 기준 고유 집계라 부풀지 않는다.
      - 경위: 한때(#192, 2026-09-09 오전) producerOrg가 아닌 공동출원인을 기업 편중
        우려로 전부 `unverified`로 되돌렸다가(#193으로 다시 뒤집힘) 완화로 확정했고,
        여기에 원물만 좁히는 단서를 얹은 것이 현재 규칙이다.
