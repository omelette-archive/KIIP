...
4. 복수 출원인이 모두 같은 결과일 때 그 결과를 사용한다. 지역이 서로 다르면
   ...
   2. 그 외 **공동출원인 중 누구든 하나라도 `inside`**면 `coapplicant_inside`로 인정한다.
      ...
      - 원물/가공품 판정은 `02-normalize-items/lib/derivedRawItems.js`의
        `specialtyStageOf(고시명칭)` — `신선한/미가공/무가공/살아있는` 수식어면 `raw`,
        가공 접미어(`~차`·`~장`·`~주` 등)나 통짜 가공품명이면 `processed`, **판정 못 하면**
        **`null`**이다(「쌀」·「소고기」·「굴」처럼 수식어도 접미어도 없는 이름).
      - `null`은 완화 쪽으로 처리한다 — 판정 못 한 것을 근거로 건수를 깎지 않는다.
