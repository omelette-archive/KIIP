...
- **새 후보**: data.go.kr `지식재산처_등록원부 실시간 정보 조회 서비스`(`15124946`, REST 타입,
  개발·운영 모두 자동승인) — 등록번호 기준으로 특허/실용/디자인/상표 등록원부의 출원인·대리인
  정보를 제공한다고 설명돼 있음. 주소 포함 여부는 실키 검증 전이라 미확정 — 상세는 이슈 #11
  코멘트(2026-08-10) 참고.
- 특허 쪽에는 서지상세(`getBibliographyDetailInfoSearch`)가 있지만 상표용 서지상세는 레퍼런스
  리포에 구현되어 있지 않음 (공식 API에 존재하는지도 별도 확인 필요).

→ 후속 작업: 위 등록원부 API를 활용신청하고 실키로 주소 필드 존재 여부를 확인한다(#11). 농사로
지역브랜드 602건 subset은 `aplcnoInfo`를 출원번호로 직접 조인할 수 있지만, 이 지역은 출원인
주소가 아닌 브랜드 연관 지역이므로 #11의 결과를 대체하지 않고 별도 `regionalBrand*` 지표로만
사용한다(#24의 검증자료로 이미 반영 중).
...
- **후보 발견**(2026-08-10): data.go.kr `지식재산처_KIPRISPlus_상표 분류코드 변동 이력_REST API`
  (`15059142`, LINK 타입, 개발·운영 모두 자동승인)가 `출원번호` 키로 `지정상품명`·`NICE분류판`·
  `상품분류코드`·`유사군코드`를 제공한다고 설명돼 있음. `getWordSearch`의 `applicationNumber`와
  바로 조인 가능해 보이나 LINK 타입이라 실제 엔드포인트는 아직 미확인 — 이슈 #12 코멘트
  (2026-08-10) 참고.
