...
- **해결**: 위 등록원부 API(`getMarkHistory`)가 `productList[].productClsCd`(NICE 상품분류코드)·
  `productList[].desProduct`(지정상품명)도 함께 제공함을 2026-08-11 실키로 확인했다(실측:
  `"신선한사과"`, `"미가공사과"`, `"미가공사과(강원도양양군에서생산된해풍사과에한함)"` — 공백
  없이 붙어 나오는 형태라 02단계 고시명칭 사전과 대조할 때 정규화 필요). `상표 분류코드 변동
