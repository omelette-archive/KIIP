...
| 임산물DB백과(한국임업진흥원, #114) | 인증 불필요, 일반 User-Agent 헤더 필요(없으면 보안정책 차단) | 🟡 실응답 검증 완료(대분류 8·수실류 하위 13품목·상세정보), 수집기 미구현 — 나중에 데이터 반영 예정 |
| 품질인증수산물(국립수산물품질관리원, #114) | data.go.kr 신청 필요, 파라미터명이 표준과 다름(`cert_key`) | ✅ 수집기 구현 완료(`lib/nfqsClient.js`, 실키 290건 확인) — 기본 `--sources`에는 아직 안 넣음(명시적으로 지정해야 수집됨) |
