...
**AI(Anthropic API)는 이 단계에서 전혀 쓰지 않는다.** 규칙으로 확정 못한 행은
`review-required.csv`에 후보·사유와 함께 남기고, 필요하면 사람이 별도로(수동으로)
검토한다 — 이 검토를 자동화하는 스크립트/클라이언트는 의도적으로 만들지 않는다.
...
├── lib/
│   ├── loadEnv.js          .env 로더 (03에서 포팅, 이 단계에서는 미사용)
│   ├── fetchWithRetry.js   재시도/타임아웃/키마스킹 (03에서 포팅, 이 단계에서는 미사용)
